    ...
    T* __restrict__ output)
{
    ...
    // Process mini-vectors using direct memory access (works for all bitwidths)
...
DatasetResult processDataset(const DatasetInfo& dataset, int partition_size,
    ...
    DatasetResult result;
    if (dataset.is_signed) {
        result = runModelStatistics<int32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs);
    } else if (dataset.is_uint64) {
        result = runModelStatistics<uint64_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs);
    } else {
        result = runModelStatistics<uint32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs);
    }
    return result;
}
