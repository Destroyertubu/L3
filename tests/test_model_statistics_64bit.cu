/**
 * L3 Model Statistics E2E Test - 64-bit Vectorized Optimization
 *
 * This is a specialized version optimized for uint64/int64 data types using
 * vectorized bit extraction (4 values extracted at once).
 *
 ...
 * - V4 Optimized Interleaved Decoder with 64-bit vectorized extraction
 ...
 *
 * Date: 2025-12-09
 * Updated: 2025-01-27 - Added 64-bit vectorized extraction optimization
 */
...
// ============================================================================
// Vectorized 64-bit Extraction Function (extracts 4 consecutive uint64 values)
// For uint64/int64 types, we need to read 3 32-bit words to support up to 64-bit
// cross-boundary extraction.
// ============================================================================
__device__ __forceinline__ void extract_vectorized_4_64bit_rt(
    ...
    uint64_t& v0, uint64_t& v1, uint64_t& v2, uint64_t& v3)
{
    ...
    const bool needs_third_word = (bits > 32);

    // Lambda to extract a single value at a given index
    auto extract_one = [&](int value_idx) -> uint64_t {
        ...
        // Load two or three consecutive 32-bit words and combine
        uint32_t w0 = __ldg(&words[word_idx]);
        uint32_t w1 = __ldg(&words[word_idx + 1]);
        uint64_t combined = (static_cast<uint64_t>(w1) << 32) | w0;
        uint64_t extracted = combined >> bit_in_word;

        // For bits > 32, we may need a third word
        if (needs_third_word && bit_in_word > 0 && (64 - bit_in_word) < bits) {
            uint32_t w2 = __ldg(&words[word_idx + 2]);
            int bits_from_first_two = 64 - bit_in_word;
            extracted |= (static_cast<uint64_t>(w2) << bits_from_first_two);
        }
    ...
    T* __restrict__ output)
{
    ...
    // Pre-compute needs_third_word flag
    ...
    // Process mini-vectors using vectorized extraction (4 values at a time)
    for (int mv_idx = warp_id_in_block; mv_idx < num_mv; mv_idx += warps_per_block) {
        ...
        // Use vectorized extraction for 64-bit types - process 4 values at a time
        if constexpr (sizeof(T) > 4) {
            if (delta_bits == 0) {
                // Zero bits - all values are base/predicted
                #pragma unroll
                ...
                // Non-zero bits - use vectorized 64-bit extraction (4 values at a time)
                #pragma unroll
                for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                    uint64_t vals[4];
                    extract_vectorized_4_64bit_rt(interleaved_array, static_cast<uint64_t>(local_bit), delta_bits,
            ...
            // Original path for uint32/int32 types - process 1 value at a time
            #pragma unroll
            for (int v = 0; v < VALUES_PER_THREAD; v++) {
                int global_idx = mv_start_global + v * WARP_SIZE_V4 + lane_id;
                int local_idx_in_partition = mv_idx * MINI_VECTOR_SIZE + v * WARP_SIZE_V4 + lane_id;
                ...
                if (model_type == MODEL_FOR_BITPACK) {
                    if (delta_bits == 0) {
                        ...
                        uint64_t extracted = Vertical::extract_branchless_64_rt(interleaved_array, local_bit, delta_bits);
...
DatasetResult processDataset(const DatasetInfo& dataset, int partition_size,
    ...
    DatasetResult result;
    if (dataset.is_signed) {
        result = runModelStatistics<int32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size);
    } else if (dataset.is_uint64) {
        result = runModelStatistics<uint64_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size);
    } else {
        result = runModelStatistics<uint32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size);
    }
    return result;
}
