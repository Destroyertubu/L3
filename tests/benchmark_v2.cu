/**
 * Benchmark V2 (Cost-Aware) Partitioning + Vertical Decompression
 */
...
void runTest(const std::vector<T>& data, const std::string& name) {
    ...
    // Create V2 partitions
    int num_partitions = 0;
    auto partitions = createPartitionsCostAware<T>(data, 2048, &num_partitions, 0);

    if (partitions.empty()) {
        std::cout << name << "|V2_FAIL|0|0|0|0|0|FAIL" << std::endl;
        return;
    }
...
int main(int argc, char** argv) {
    ...
    std::cout << "# V2 (Cost-Aware) + Vertical Results" << std::endl;
