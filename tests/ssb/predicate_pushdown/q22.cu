/**
 * @file q22.cu
 * @brief SSB Q2.2 Implementation - Predicate Pushdown Strategy
 */
...
__global__ void q22_predicate_kernel(
    const uint32_t* __restrict__ lo_orderdate, const uint32_t* __restrict__ lo_partkey,
...
{
    uint32_t orderdate[ITEMS_PER_THREAD], partkey[ITEMS_PER_THREAD], suppkey[ITEMS_PER_THREAD], revenue[ITEMS_PER_THREAD];
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        ...
        if (year_idx >= 0 && year_idx < NUM_YEARS && p_brand < NUM_BRANDS) {
            atomicAdd(&agg_revenue[year_idx * NUM_BRANDS + p_brand], static_cast<unsigned long long>(revenue[i]));
        }
    }
}
...
__global__ void build_part_brand_range_ht(const uint32_t* p_partkey, const uint32_t* p_brand1, int n,
                                           uint32_t brand_min, uint32_t brand_max, uint32_t* ht_k, uint32_t* ht_v, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x; if (idx >= n) return;
    ...
    for (int p = 0; p < ht_size; ++p) {
        int s = (slot + p) % ht_size;
        uint32_t old = atomicCAS(&ht_k[s], static_cast<uint32_t>(HT_EMPTY), key);
        if (old == static_cast<uint32_t>(HT_EMPTY) || old == key) { ht_v[s] = brand; return; }
    }
}

void runQ22PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    ...
    build_supplier_region_ht<<<(S_LEN+bs-1)/bs, bs>>>(data.d_s_suppkey, data.d_s_region, S_LEN, S_REGION_ASIA, ht_s_keys, ht_s_size);
    build_part_brand_range_ht<<<(P_LEN+bs-1)/bs, bs>>>(data.d_p_partkey, data.d_p_brand1, P_LEN, P_BRAND_MIN, P_BRAND_MAX, ht_p_keys, ht_p_values, ht_p_size);
    ...
    q22_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_partkey, d_suppkey, d_revenue, LO_LEN,
    ...
    std::cout << "\n=== Q2.2 Results (Predicate Pushdown) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.2 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ22PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ22PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
