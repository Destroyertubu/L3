/**
 * @file q34.cu
 * @brief SSB Q3.4 Implementation - Predicate Pushdown Strategy
 *
 * Uses partition bounds to skip partitions outside Dec 1997.
 */
...
__global__ void q34_predicate_kernel(
    ...
    int num_rows,
    const uint32_t* __restrict__ ht_d_keys, int ht_d_size,
...
__global__ void build_date_dec1997_ht(const uint32_t* d_datekey, int n, uint32_t start, uint32_t end,
                                       uint32_t* ht_k, int ht_size) {
...
__global__ void build_city_filter_ht(const uint32_t* keys, const uint32_t* cities, int n,
...
void runQ34PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    ...
    int bs = 256;
    build_date_dec1997_ht<<<(D_LEN+bs-1)/bs, bs>>>(data.d_d_datekey, D_LEN, DATE_DEC1997_START, DATE_DEC1997_END, ht_d_keys, ht_d_size);
    build_city_filter_ht<<<(C_LEN+bs-1)/bs, bs>>>(data.d_c_custkey, data.d_c_city, C_LEN, CITY_UK1, CITY_UK5, ht_c_keys, ht_c_values, ht_c_size);
    build_city_filter_ht<<<(S_LEN+bs-1)/bs, bs>>>(data.d_s_suppkey, data.d_s_city, S_LEN, CITY_UK1, CITY_UK5, ht_s_keys, ht_s_values, ht_s_size);
    ...
    q34_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_revenue, LO_LEN,
        ht_d_keys, ht_d_size, ht_c_keys, ht_c_values, ht_c_size, ht_s_keys, ht_s_values, ht_s_size, d_agg_revenue);
    cudaDeviceSynchronize();
    timer.stop();
    ...
    std::cout << "\n=== Q3.4 Results (Predicate Pushdown) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.4 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ34PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ34PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
