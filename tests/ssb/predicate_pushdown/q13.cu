/**
 * @file q13.cu
 * @brief SSB Q1.3 Implementation - Predicate Pushdown Strategy
 */
...
// Week 6 of 1994: Feb 6-12
constexpr uint32_t DATE_WEEK6_MIN = 19940206;
constexpr uint32_t DATE_WEEK6_MAX = 19940212;

__global__ void q13_predicate_kernel(
    ...
    int num_rows, unsigned long long* revenue)
{
    ...
    BlockPredAndGTE(orderdate, DATE_WEEK6_MIN, selection_flags, block_items);
    BlockPredAndLTE(orderdate, DATE_WEEK6_MAX, selection_flags, block_items);

    BlockPredLoad(lo_discount + block_start, discount, block_items, selection_flags);
    BlockPredAndGTE(discount, static_cast<uint32_t>(5), selection_flags, block_items);
    BlockPredAndLTE(discount, static_cast<uint32_t>(7), selection_flags, block_items);
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        if (selection_flags[i]) local_sum += static_cast<unsigned long long>(extendedprice[i]) * static_cast<unsigned long long>(discount[i]);
    }
...
void runQ13PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    acc_orderdate.getCandidatePartitions(DATE_WEEK6_MIN, DATE_WEEK6_MAX, candidate_partitions);

    std::cout << "Predicate pushdown: " << candidate_partitions.size() << "/" << acc_orderdate.getNumPartitions() << " partitions" << std::endl;
    ...
    if (total_elements == 0) {
        ...
        std::cout << "\n=== Q1.3 Results (Predicate Pushdown) ===" << std::endl;
    ...
    q13_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_discount, d_quantity, d_extendedprice, total_elements, d_revenue);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.3 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ13PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ13PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
