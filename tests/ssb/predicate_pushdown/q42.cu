/**
 * @file q42.cu
 * @brief SSB Q4.2 Implementation - Predicate Pushdown Strategy
 */
...
__global__ void q42_predicate_kernel(
...
void runQ42PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    ...
    q42_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_partkey, d_revenue, d_supplycost, LO_LEN,
    ...
    std::cout << "\n=== Q4.2 Results (Predicate Pushdown) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.2 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ42PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ42PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
