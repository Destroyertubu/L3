/**
 * @file q43.cu
 * @brief SSB Q4.3 Implementation - Predicate Pushdown Strategy
 */
...
__global__ void q43_predicate_kernel(
    const uint32_t* __restrict__ lo_orderdate, const uint32_t* __restrict__ lo_suppkey,
    const uint32_t* __restrict__ lo_partkey, const uint32_t* __restrict__ lo_revenue,
...
void runQ43PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    ...
    uint32_t *d_orderdate, *d_suppkey, *d_partkey, *d_revenue, *d_supplycost;
    ...
    q43_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_suppkey, d_partkey, d_revenue, d_supplycost, LO_LEN,
    ...
    std::cout << "\n=== Q4.3 Results (Predicate Pushdown) ===" << std::endl;
    ...
    cudaFree(d_orderdate); cudaFree(d_suppkey); cudaFree(d_partkey); cudaFree(d_revenue); cudaFree(d_supplycost);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.3 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ43PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ43PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
