/**
 * @file q13.cu
 * @brief SSB Q1.3 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q13_fused_kernel(
    ...
    unsigned long long* revenue)
{
    ...
    // Week 6 of 1994: Feb 6-12, 1994 (dates 19940206-19940212)
    BlockLoad(lo_orderdate + block_start, orderdate, block_items);
    BlockPredAndGTE(orderdate, static_cast<uint32_t>(19940206), selection_flags, block_items);
    BlockPredAndLTE(orderdate, static_cast<uint32_t>(19940212), selection_flags, block_items);
...
void runQ13Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    for (int offset = 0; offset < total_elements; offset += CHUNK_SIZE) {
        ...
        q13_fused_kernel<<<grid_size, BLOCK_THREADS>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.3 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ13Fused(data, timing);
