/**
 * @file q41.cu
 * @brief SSB Q4.1 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q41_fused_kernel(
...
__global__ void build_part_mfgr_ht(const uint32_t* p_partkey, const uint32_t* p_mfgr, int n, uint32_t mfgr1, uint32_t mfgr2, uint32_t* ht_k, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
...
void runQ41Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q41_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_partkey, d_revenue, d_supplycost, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.1 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ41Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ41Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
