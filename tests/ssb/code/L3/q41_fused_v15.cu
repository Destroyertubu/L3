/**
 * SSB Q4.1 - V13 (L3 V4 Vectorized Decoder)
 *
...
constexpr long long EXPECTED_TOTAL_Q41 = 59526851654LL;

__global__ void q41_fused_kernel_v15(
    ...
    long long* __restrict__ agg_profit)
{
    ...
    if (num_full_mv > 0) {
        ...
        // 1) Customer join + filter (c_region = 'AMERICA'), payload: c_nation
        ...
        if (__any_sync(FULL_MASK, valid_mask != 0)) {
            // 2) Supplier filter (s_region = 'AMERICA')
                        ...
                        if (valid_mask & bit) {
                            ...
                            if (kv == 0) {
                                ...
                                if ((unsigned)year_idx < (unsigned)AGG_NUM_YEARS) {
                                    agg_idx[v] = static_cast<uint16_t>(year_idx * AGG_NUM_NATIONS + c_nation[v]);
                                } else {
                                    valid_mask &= ~bit;
                                }
                            }
    ...
    for (int t = lane_id; t < tail_size; t += V15_WARP_SIZE) {
        ...
        if (__ldg(&ht_p[partkey % ht_p_len]) == 0) continue;
        ...
        if (year_idx >= 0 && year_idx < AGG_NUM_YEARS && cn < AGG_NUM_NATIONS) {
            ...
            long long profit = static_cast<long long>(rev) - static_cast<long long>(sc);
            atomicAdd(reinterpret_cast<unsigned long long*>(&agg_profit[year_idx * AGG_NUM_NATIONS + cn]),
                      static_cast<unsigned long long>(profit));
        }
    }
}

void runQ41FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildCustomerHashTable_Region(data.d_c_custkey, data.d_c_region, data.d_c_nation, C_LEN,
                                  &ht_c, &ht_c_len, REGION_AMERICA);
    buildSupplierHashTable_Region(data.d_s_suppkey, data.d_s_region, S_LEN,
                                  &ht_s, &ht_s_len, REGION_AMERICA);
    buildPartHashTable_MfgrRange(data.d_p_partkey, data.d_p_mfgr, P_LEN,
                                 &ht_p, &ht_p_len, 1, 2);
    ...
    q41_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":41,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << total_profit
                  << ",\"status\":\"" << (total_profit == EXPECTED_TOTAL_Q41 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
