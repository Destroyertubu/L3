/**
 * SSB Q2.1 - V13 (L3 V4 Vectorized Decoder, 2048 values/partition)
 *
...
constexpr unsigned long long EXPECTED_TOTAL_Q21 = 29936907152ULL;

__global__ void q21_fused_kernel_v15(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    if (num_full_mv > 0) {
        ...
        // 1) Supplier filter (s_region = 'AMERICA')
        ...
        if (__any_sync(FULL_MASK, valid_mask != 0)) {
            // 2) Part join + filter (p_category = 'MFGR#12'), payload: brand
...
void runQ21FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildPartHashTable_Category(data.d_p_partkey, data.d_p_category, data.d_p_brand1, P_LEN,
                                &ht_p, &ht_p_len, CATEGORY_MFGR12);
    ...
    q21_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
        data.lo_suppkey, data.lo_partkey, data.lo_orderdate, data.lo_revenue,
        num_partitions, total_rows,
        ht_s, ht_s_len, ht_p, ht_p_len, ht_d, ht_d_len, d_agg);
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        // Kernel only (tail processing is fused into main kernel)
        q21_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
            data.lo_suppkey, data.lo_partkey, data.lo_orderdate, data.lo_revenue,
            num_partitions, total_rows,
            ht_s, ht_s_len, ht_p, ht_p_len, ht_d, ht_d_len, d_agg);
        ...
        for (auto v : h_agg) total_revenue += v;
        ...
        std::cout << "{\"query\":21,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"status\":\"" << (total_revenue == EXPECTED_TOTAL_Q21 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
