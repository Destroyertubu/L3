/**
 * SSB Q1.3 - V13 (L3 V4 Vectorized Decoder, 2048 values/partition)
 *
...
constexpr unsigned long long EXPECTED_REVENUE_Q13 = 4708750603ULL;

__global__ void q13_fused_kernel_v15(
    ...
    unsigned long long* __restrict__ global_revenue)
{
    ...
    // Filter: weeknuminyear = 6, year = 1994 (Feb 6-12, 1994)
    #pragma unroll
    ...
    // Filter: discount BETWEEN 5 AND 7
    #pragma unroll
    for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
...
__global__ void q13_tail_kernel_v15(
    const CompressedDataVertical<uint32_t> lo_orderdate,
    const CompressedDataVertical<uint32_t> lo_quantity,
    const CompressedDataVertical<uint32_t> lo_discount,
    const CompressedDataVertical<uint32_t> lo_extendedprice,
    ...
    unsigned long long* __restrict__ global_revenue)
{
    ...
    if (orderdate < WEEK6_1994_START || orderdate > WEEK6_1994_END) return;
...
void runQ13FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":13,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << h_revenue
                  << ",\"status\":\"" << (h_revenue == EXPECTED_REVENUE_Q13 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
