/**
 * SSB Q3.2 - V13 (L3 V4 Vectorized Decoder)
 *
...
constexpr unsigned long long EXPECTED_TOTAL_Q32 = 4997799844ULL;

__global__ void q32_fused_kernel_v15(
...
void runQ32FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildCustomerHashTable_Nation(data.d_c_custkey, data.d_c_nation, data.d_c_city, C_LEN, &ht_c, &ht_c_len, NATION_US);
    buildSupplierHashTable_NationCity(data.d_s_suppkey, data.d_s_nation, data.d_s_city, S_LEN, &ht_s, &ht_s_len, NATION_US);
    ...
    q32_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":32,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << total
                  << ",\"status\":\"" << (total == EXPECTED_TOTAL_Q32 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
