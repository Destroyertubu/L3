/**
 * SSB Q4.3 - V13 (L3 V4 Vectorized Decoder)
 *
...
constexpr long long EXPECTED_TOTAL_Q43 = 64631044LL;

__global__ void q43_fused_kernel_v15(
    ...
    long long* __restrict__ agg_profit)
{
    ...
    if (num_full_mv > 0) {
        ...
        uint16_t s_city[V15_VALUES_PER_THREAD];
        uint16_t p_brand[V15_VALUES_PER_THREAD];
        uint32_t agg_idx[V15_VALUES_PER_THREAD];
        uint64_t valid_mask = 0;

        // 1) Customer filter (c_nation = 'UNITED STATES')
        ...
        for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
            ...
            if (global_idx < total_rows) {
                if (__ldg(&ht_c[vals[v] % ht_c_len]) != 0) {
                    valid_mask |= (1ull << v);
                }
            }
        }
        ...
        if (__any_sync(FULL_MASK, valid_mask != 0)) {
            // 2) Supplier join + filter (s_nation = 'UNITED STATES'), payload: s_city
            ...
            for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
                ...
                if (valid_mask & bit) {
                    ...
                    if (kv == 0) {
                        ...
                        uint32_t sc = static_cast<uint32_t>(kv >> 32);
                        if (sc < AGG_NUM_CITIES) {
                            s_city[v] = static_cast<uint16_t>(sc);
                        } else {
                            valid_mask &= ~bit;
                        }
                    }
            ...
            if (__any_sync(FULL_MASK, valid_mask != 0)) {
                // 3) Part join + filter (p_category = 'MFGR#14'), payload: p_brand
                ...
                for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
                    ...
                    if (valid_mask & bit) {
                        ...
                        if (kv == 0) {
                            ...
                            uint32_t pb = static_cast<uint32_t>(kv >> 32);
                            if (pb < AGG_NUM_BRANDS) {
                                p_brand[v] = static_cast<uint16_t>(pb);
                            } else {
                                valid_mask &= ~bit;
                            }
                        }
                ...
                if (__any_sync(FULL_MASK, valid_mask != 0)) {
                    // 4) Date join + filter (1997, 1998), payload: year
                    ...
                    for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
                        ...
                        if (valid_mask & bit) {
                            ...
                            if (kv == 0) {
                                ...
                                agg_idx[v] = year_mod * (AGG_NUM_CITIES * AGG_NUM_BRANDS) +
                    ...
                    if (__any_sync(FULL_MASK, valid_mask != 0)) {
                        ...
                        while (m) {
                            ...
                            atomicAdd(reinterpret_cast<unsigned long long*>(&agg_profit[agg_idx[v]]),
    ...
    for (int t = lane_id; t < tail_size; t += V15_WARP_SIZE) {
        ...
        if (__ldg(&ht_c[custkey % ht_c_len]) == 0) continue;
        ...
        if (kv_s == 0) continue;
        uint32_t sc_val = static_cast<uint32_t>(kv_s >> 32);
        ...
        if (kv_p == 0) continue;
        uint32_t pb = static_cast<uint32_t>(kv_p >> 32);
        ...
        if (sc_val < AGG_NUM_CITIES && pb < AGG_NUM_BRANDS) {
            int year_mod = (static_cast<int>(yr) - 1992) & 1;  // 1998->0, 1997->1
            ...
            long long profit = static_cast<long long>(rev) - static_cast<long long>(sc);
            int agg_i = year_mod * (AGG_NUM_CITIES * AGG_NUM_BRANDS) + sc_val * AGG_NUM_BRANDS + pb;
...
void runQ43FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildCustomerHashTable_NationOnly(data.d_c_custkey, data.d_c_nation, C_LEN,
                                      &ht_c, &ht_c_len, NATION_US);
    buildSupplierHashTable_NationCity(data.d_s_suppkey, data.d_s_nation, data.d_s_city, S_LEN,
                                      &ht_s, &ht_s_len, NATION_US);
    buildPartHashTable_CategoryBrand(data.d_p_partkey, data.d_p_category, data.d_p_brand1, P_LEN,
                                     &ht_p, &ht_p_len, CATEGORY_MFGR14);
    ...
    q43_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":43,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << total_profit
                  << ",\"status\":\"" << (total_profit == EXPECTED_TOTAL_Q43 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
