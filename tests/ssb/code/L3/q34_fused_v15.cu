/**
 * SSB Q3.4 - V13 (L3 V4 Vectorized Decoder)
 *
...
constexpr uint32_t CITY_UK1 = 249;
constexpr uint32_t CITY_UK5 = 244;
constexpr unsigned long long EXPECTED_TOTAL_Q34 = 3086915ULL;

__global__ void q34_fused_kernel_v15(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    const uint64_t* ht_c64 = reinterpret_cast<const uint64_t*>(ht_c);
	    ...
	    if (num_full_mv > 0) {
	        ...
	        uint32_t agg_idx[V15_VALUES_PER_THREAD];  // cc * 250 + sc, then add year offset
	        uint64_t valid_mask = 0;

	        // 1) Supplier join + filter first, payload: s_city
	        ...
	        if (__any_sync(FULL_MASK, valid_mask != 0)) {
	            // 2) Customer join, payload: c_city -> build (cc, sc) composite index
	            ...
	            for (int v = 0; v < V15_VALUES_PER_THREAD; v++) {
	                ...
	                if (valid_mask & bit) {
	                    ...
	                    if (kv == 0) {
	                        ...
	                        uint32_t cc = static_cast<uint32_t>(kv >> 32);
	                        if (cc < AGG_NUM_CITIES) {
	                            agg_idx[v] = cc * AGG_NUM_CITIES + static_cast<uint32_t>(s_city[v]);
	                        } else {
	                            valid_mask &= ~bit;
	                        }
	                    }
                    ...
                    if (valid_mask & bit) {
                        ...
                        if (kv == 0) {
                            ...
                            if ((unsigned)year_idx < (unsigned)AGG_NUM_YEARS) {
                                agg_idx[v] = year_idx * AGG_NUM_CITIES * AGG_NUM_CITIES + agg_idx[v];
                            } else {
                                valid_mask &= ~bit;
                            }
                        }
	    ...
	    for (int t = lane_id; t < tail_size; t += V15_WARP_SIZE) {
	        ...
	        if (kv_s == 0) continue;
	        uint32_t sc = static_cast<uint32_t>(kv_s >> 32);
	        ...
	        if (kv_c == 0) continue;
	        uint32_t cc = static_cast<uint32_t>(kv_c >> 32);
        ...
        if (year_idx >= 0 && year_idx < AGG_NUM_YEARS &&
            cc < AGG_NUM_CITIES && sc < AGG_NUM_CITIES) {
            uint32_t rev = decode_tail_value_v15_opt<BW_REVENUE>(lo_revenue, partition_idx, value_idx);
            int idx = year_idx * AGG_NUM_CITIES * AGG_NUM_CITIES + cc * AGG_NUM_CITIES + sc;
            atomicAdd(&agg_revenue[idx], static_cast<unsigned long long>(rev));
        }
    }
}

void runQ34FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildCustomerHashTable_Cities(data.d_c_custkey, data.d_c_city, C_LEN, &ht_c, &ht_c_len, CITY_UK1, CITY_UK5);
    buildSupplierHashTable_Cities(data.d_s_suppkey, data.d_s_city, S_LEN, &ht_s, &ht_s_len, CITY_UK1, CITY_UK5);
    buildDateHashTable_YearMonth(data.d_d_datekey, data.d_d_year, D_LEN, &ht_d, &ht_d_len, 199712);
    ...
    q34_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":34,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << total
                  << ",\"status\":\"" << (total == EXPECTED_TOTAL_Q34 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
