/**
 * SSB Q2.2 - V13 (L3 V4 Vectorized Decoder, 2048 values/partition)
 *
...
constexpr unsigned long long EXPECTED_TOTAL_Q22 = 6211591877ULL;

__global__ void q22_fused_kernel_v15(
...
void runQ22FusedV13(SSBDataCompressedVertical& data, float time_h2d) {
    ...
    buildSupplierHashTable_Region(data.d_s_suppkey, data.d_s_region, S_LEN, &ht_s, &ht_s_len, REGION_ASIA);
    buildPartHashTable_BrandRange(data.d_p_partkey, data.d_p_brand1, P_LEN, &ht_p, &ht_p_len, BRAND_MFGR2221, BRAND_MFGR2228);
    ...
    q22_fused_kernel_v15<<<num_partitions, V15_THREADS_PER_BLOCK>>>(
    ...
    for (int run = 0; run < RUNS; run++) {
        ...
        std::cout << "{\"query\":22,\"version\":\"v14\",\"run\":" << run
                  << ",\"time_h2d\":" << time_h2d
                  ...
                  << ",\"result\":" << total
                  << ",\"status\":\"" << (total == EXPECTED_TOTAL_Q22 ? "PASSED" : "FAILED") << "\"}" << std::endl;
    }
