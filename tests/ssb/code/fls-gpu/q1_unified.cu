...
from lineorder
where lo_orderdate >= 19930101 and lo_orderdate <= 19931231 and lo_discount>=1
and lo_discount<=3
and lo_quantity<25;
...
__global__ void QueryKernel(const int*          enc_lo_orderdate,
	...
	BlockPredGT<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
	    items, 19930100 - query_mtd.lo_orderdate_min, selection_flags, num_tile_items);
	BlockPredAndLTE<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
	    items, 19931231 - query_mtd.lo_orderdate_min, selection_flags, num_tile_items);
	int local_any = 0;
	#pragma unroll
...
float query(int*                         lo_orderdate,
	...
	/*Check the result*/
	FLS_SHOW(revenue)
	// Disabled result check for SF=10 data comparison
	// if (revenue != query_mtd.result) { throw std::runtime_error("RESULT INCOREECT!"); }
	std::cout << "{\"query\":11,\"run\":0,\"time_metadata\":0,\"time_ht_build\":0,\"time_kernel\":" << time_query << ",\"time_total\":" << time_query << "}" << std::endl;
