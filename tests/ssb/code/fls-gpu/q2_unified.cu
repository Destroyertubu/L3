...
void runQuery(int*                         lo_orderdate,
	...
	// Timing starts here for fair comparison
	...
	// Disabled for benchmark comparison
	// ASSERT_EQ(result_of_query.size(), ssb::ssb_q21_10.reuslt.size());
	// ASSERT_EQ(result_of_query, ssb::ssb_q21_10.reuslt);

	std::cout << "{\"query\":21,\"run\":0,\"time_metadata\":0,\"time_ht_build\":0,\"time_kernel\":" << time_query << ",\"time_total\":" << time_query << "}" << std::endl;
...
int main(int argc, char* argv[]) {
	...
	// Disabled for benchmark - data should already be sorted
