/**
 * @file q13.cu
 * @brief SSB Q1.3 Implementation - Decompress First Strategy
 *
 * Query:
...
/**
 * @brief Q1.3 query kernel with Crystal-style block processing
 *
 * Processes LINEORDER table with predicates:
 * - lo_orderdate in week 6 of 1994 (date between 19940207 and 19940213)
 * - lo_discount between 5 and 7
 * - lo_quantity between 26 and 35
 *
 * Computes: SUM(lo_extendedprice * lo_discount)
 */
__global__ void q13_kernel(
    ...
    unsigned long long* revenue)
{
    ...
    // Load orderdate and filter by week 6 of 1994 (Feb 7-13, 1994)
    BlockLoad(lo_orderdate + block_start, orderdate, block_items);
    BlockPredAndGTE(orderdate, static_cast<uint32_t>(19940207), selection_flags, block_items);
    BlockPredAndLTE(orderdate, static_cast<uint32_t>(19940213), selection_flags, block_items);

    // Load and filter by discount (5-7)
...
// ============================================================================
// Q1.3 Execution Function
// ============================================================================

/**
 * @brief Execute Q1.3 with decompress-first strategy
 */
void runQ13DecompressFirst(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    q13_kernel<<<grid_size, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q1.3 Results (Decompress-First) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.3 - Decompress First Strategy ===" << std::endl;
    ...
    // Run Q1.3
    QueryTiming timing;
    runQ13DecompressFirst(data, timing);

    // Warmup and benchmark
    std::cout << "\n=== Benchmark (3 runs) ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ13DecompressFirst(data, t);
