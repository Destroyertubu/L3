/**
 * @file q34.cu
 * @brief SSB Q3.4 Implementation - Decompress First Strategy
 *
 * Query:
...
void runQ34DecompressFirst(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    timer.start();
    q34_kernel<<<(LO_LEN+TILE_SIZE-1)/TILE_SIZE, BLOCK_THREADS>>>(d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_revenue, LO_LEN,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.4 - Decompress First ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);
    QueryTiming timing;
    runQ34DecompressFirst(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ34DecompressFirst(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
