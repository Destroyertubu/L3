/**
 * @file q22.cu
 * @brief SSB Q2.2 Implementation - Random Access Optimization Strategy
 *
 * Query:
...
// Q2.2 specific: brand range 2221-2228, ASIA region
constexpr uint32_t P_BRAND_MIN = 221;  // MFGR#2221 encoded
constexpr uint32_t P_BRAND_MAX = 228;  // MFGR#2228 encoded
 ...
 */
__global__ void probeSingleFilterQ22Kernel(
 ...
 */
__global__ void probePartWithIndicesQ22Kernel(
...
__global__ void build_part_brand_range_ht_q22(
...
__global__ void build_supplier_region_ht_q22(
...
void runQ22RandomAccess(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    build_part_brand_range_ht_q22<<<(P_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_p_partkey, data.d_p_brand1, P_LEN, P_BRAND_MIN, P_BRAND_MAX,
    ...
    probeSingleFilterQ22Kernel<<<grid_stage1, block_size>>>(
    ...
    if (h_num_stage1 == 0) {
        std::cout << "\n=== Q2.2 Results (Staged Random Access) ===" << std::endl;
    ...
    probePartWithIndicesQ22Kernel<<<grid_stage2, block_size>>>(
    ...
    aggregateQ22Kernel<<<grid_agg, block_size>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.2 - Random Access Optimization ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ22RandomAccess(data, timing);

    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ22RandomAccess(data, t);
