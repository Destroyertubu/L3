/**
 * @file q12.cu
 * @brief SSB Q1.2 Implementation - Random Access Optimization Strategy
 *
 * Query:
...
/**
 * @brief Secondary filter kernel for Q1.2 (quantity range instead of < threshold)
 */
__global__ void filterSecondaryQ12Kernel(
...
/**
 * @brief Run Q1.2 with random access optimization
 */
void runQ12RandomAccess(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    timer.start();
    launchDateRangeFilter(d_orderdate, total_rows, Q12_DATE_MIN, Q12_DATE_MAX,
    ...
    if (h_num_passing == 0) {
        std::cout << "\n=== Q1.2 Results (Random Access) ===" << std::endl;
    ...
    filterSecondaryQ12Kernel<<<grid_size, FILTER_BLOCK_SIZE, 0, stream>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.2 - Random Access Optimization ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ12RandomAccess(data, timing);

    // Benchmark runs
    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ12RandomAccess(data, t);
