/**
 * @file q23.cu
 * @brief SSB Q2.3 Implementation - Random Access Optimization Strategy
 *
 * Query:
...
// Q2.3 specific: exact brand MFGR#2239, EUROPE region
 ...
 */
__global__ void probeSingleFilterQ23Kernel(
 ...
 */
__global__ void probePartWithIndicesQ23Kernel(
...
/**
 * @brief Final aggregation with date hash lookup for Q2.3
 */
__global__ void aggregateQ23Kernel(
    ...
    for (int i = tid; i < num_rows; i += blockDim.x * gridDim.x) {
        ...
        if (found) {
            ...
            if (year_idx >= 0 && year_idx < NUM_YEARS) {
                atomicAdd(&agg_revenue[year_idx], static_cast<unsigned long long>(rev));
            }
        }
    }
}
...
__global__ void build_date_ht_q23(const uint32_t* d_datekey, const uint32_t* d_year, int num_rows,
...
__global__ void build_part_exact_brand_ht_q23(
    const uint32_t* p_partkey, const uint32_t* p_brand1, int num_rows,
...
__global__ void build_supplier_region_ht_q23(
...
void runQ23RandomAccess(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    build_date_ht_q23<<<(D_LEN + block_size - 1) / block_size, block_size>>>(
    ...
    build_part_exact_brand_ht_q23<<<(P_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_p_partkey, data.d_p_brand1, P_LEN, P_BRAND_TARGET, ht_p_keys, ht_p_size);
    ...
    build_supplier_region_ht_q23<<<(S_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_s_suppkey, data.d_s_region, S_LEN, S_REGION_EUROPE, ht_s_keys, ht_s_size);
    ...
    probeSingleFilterQ23Kernel<<<grid_stage1, block_size>>>(
    ...
    probePartWithIndicesQ23Kernel<<<grid_stage2, block_size>>>(
    ...
    aggregateQ23Kernel<<<grid_agg, block_size>>>(
        d_orderdate, d_revenue, h_num_stage2,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.3 - Random Access Optimization ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ23RandomAccess(data, timing);

    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ23RandomAccess(data, t);
