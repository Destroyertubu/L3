/**
 * @file q11_optimized.cu
 * @brief SSB Q1.1 Implementation - OPTIMIZED with multi-stream parallel decompression
 *
 * Query: SUM(lo_extendedprice * lo_discount) WHERE date in 1993, discount 1-3, quantity < 25
 *
 * Q1.x queries don't use hash JOIN - they're direct filters on lineorder.
 *
 * Key insight: For Q1.x with ~14% selectivity, sequential decompression is FASTER
 * than random access because:
 * - Sequential memory access: ~1070 GB/s effective throughput
 * - Random access: ~120 GB/s due to cache misses
 *
 * Optimizations:
 * 1. Multi-stream parallel decompression of 4 columns
 * 2. Crystal-style vectorized kernel with early filtering
 */
 ...
 */
__global__ void q11_kernel_optimized(
...
void runQ11Optimized(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    // =========================================================================
    // Step 1: PARALLEL decompress all 4 columns using multiple streams
    // =========================================================================
    timer.start();
    ...
    q11_kernel_optimized<<<grid_size, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q1.1 Results (OPTIMIZED) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.1 - OPTIMIZED (Parallel Decompress) ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ11Optimized(data, timing);

    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ11Optimized(data, t);
