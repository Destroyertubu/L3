...
string lookup(string col_name) {
  ...
  if (col_name[0] == 'l') {
    ...
    return "SUPPLIER" + to_string(index) + ".bin";
  } else if (col_name[0] == 'c') {
    int index = index_of(customer, 8, col_name);
    return "CUSTOMER" + to_string(index) + ".bin";
  } else if (col_name[0] == 'p') {
    int index = index_of(part, 9, col_name);
    return "PART" + to_string(index) + ".bin";
  } else if (col_name[0] == 'd') {
    int index = index_of(date, 15, col_name);
    return "DDATE" + to_string(index) + ".bin";
  }

  return "";
}

// Load column as raw pointer (for legacy code)
template<typename T>
T* loadColumnPtr(string col_name, int num_entries) {
...
// Load column as std::vector
template<typename T>
std::vector<T> loadColumn(string col_name, int num_entries) {
  std::vector<T> h_col(num_entries);
  string filename = DATA_DIR + lookup(col_name);
  ifstream colData (filename.c_str(), ios::in | ios::binary);
  if (!colData) {
  ...
  colData.read((char*)h_col.data(), num_entries * sizeof(T));
  return h_col;
}
