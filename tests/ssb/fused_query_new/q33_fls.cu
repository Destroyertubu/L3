/**
 * @file q33_fls.cu
 * @brief SSB Q3.3 Implementation - Vertical-style FOR+BitPack
 *
 * Query:
...
// These are typically encoded as sequential integers based on alphabetical order
// For SSB data, UNITED KI* cities have codes around 231-240
constexpr int C_CITY_UK1 = 231;  // 'UNITED KI1'
constexpr int C_CITY_UK5 = 235;  // 'UNITED KI5'
...
// 2 cities * 2 cities * 6 years = 24 groups
...
// Build kernel that filters on specific cities
__global__ void build_pht_2_city_kernel(
    ...
    int* ht, int ht_len, int keys_min)
{
    ...
    if (tid < num_rows) {
        ...
        if (city == city1 || city == city2) {
            int key = keys[tid];
            // Map city to index: city1->0, city2->1
    ...
    unsigned long long* __restrict__ group_result)
{
    ...
    // Step 1: lo_custkey -> c_city_idx (0 or 1)
    {
    ...
    // Step 2: lo_suppkey -> s_city_idx (0 or 1)
    {
    ...
    // Aggregate: group_idx = c_city_idx * 2 * 6 + s_city_idx * 6 + (year - 1992)
    #pragma unroll
...
__global__ void build_city_pht_2(
    const int* __restrict__ keys,
    const int* __restrict__ cities,
    int city1, int city2,
    int num_rows,
...
void runQ33FLS(SSBDataFLS& data, FLSQueryTiming& timing) {
    ...
    std::cout << "Q3.3: " << num_blocks << " blocks" << std::endl;

    // Warmup + timed
    q33_fls_kernel<<<num_blocks, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q3.3 Average: " << timing.kernel_ms << " ms ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.3 - Vertical Style ===" << std::endl;
    SSBDataFLS data;
    data.loadAndEncode(data_dir);

    FLSQueryTiming timing;
    runQ33FLS(data, timing);
