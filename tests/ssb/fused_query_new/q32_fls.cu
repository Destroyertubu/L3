/**
 * @file q32_fls.cu
 * @brief SSB Q3.2 Implementation - Vertical-style FOR+BitPack
 *
 * Query:
...
namespace {
// US nation code = 24 (1-indexed in NATION table, under AMERICA region)
constexpr int C_NATION_US = 24;
...
// 250 cities * 250 cities * 6 years (too large, use flat grouping)
// For US: ~10 cities per nation, so ~10*10*6 = 600 groups
constexpr int NUM_CITIES = 250;  // Max city code
constexpr int NUM_YEARS = 6;
constexpr int GROUP_SIZE = NUM_CITIES * NUM_CITIES * NUM_YEARS;
}
    ...
    unsigned long long* __restrict__ group_result)
{
    ...
    // Step 1: lo_custkey -> c_city (US nation)
    {
        ...
        BlockProbeAndPHT_2<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
            items, c_city, selection_flags, c_ht, c_ht_size, c_min_key, num_tile_items);
    }
    if (IsTerm<BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) return;

    // Step 2: lo_suppkey -> s_city (US nation)
    {
        ...
        BlockProbeAndPHT_2<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
            items, s_city, selection_flags, s_ht, s_ht_size, s_min_key, num_tile_items);
    }
    if (IsTerm<BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) return;

    // Step 3: lo_orderdate -> year
    {
    ...
    for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
        if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) && selection_flags[ITEM]) {
            ...
            if (cc >= 0 && cc < NUM_CITIES && sc >= 0 && sc < NUM_CITIES && y >= 0 && y < NUM_YEARS) {
                int idx = cc * NUM_CITIES * NUM_YEARS + sc * NUM_YEARS + y;
...
void buildHashTables(SSBDataFLS& data,
    ...
    // Customer: nation=US -> city
    build_pht_2_kernel<int,int><<<(SSB_C_LEN+bs-1)/bs, bs>>>(
        data.d_c_custkey, data.d_c_city, data.d_c_nation, C_NATION_US,
        SSB_C_LEN, d_c_ht, C_HT_SIZE, c_min_key);

    // Supplier: nation=US -> city
    build_pht_2_kernel<int,int><<<(SSB_S_LEN+bs-1)/bs, bs>>>(
        data.d_s_suppkey, data.d_s_city, data.d_s_nation, S_NATION_US,
        SSB_S_LEN, d_s_ht, S_HT_SIZE, s_min_key);

    // Date: year 1992-1997
    build_pht_2_range_kernel<int,int><<<(SSB_D_LEN+bs-1)/bs, bs>>>(
...
void runQ32FLS(SSBDataFLS& data, FLSQueryTiming& timing) {
    ...
    std::cout << "Q3.2: " << num_blocks << " blocks" << std::endl;

    // Warmup + timed
    q32_fls_kernel<<<num_blocks, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q3.2 Average: " << timing.kernel_ms << " ms ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.2 - Vertical Style ===" << std::endl;
    SSBDataFLS data;
    data.loadAndEncode(data_dir);

    FLSQueryTiming timing;
    runQ32FLS(data, timing);
