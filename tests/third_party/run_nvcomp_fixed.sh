...
OUTPUT="/root/autodl-tmp/code/L3/H20/SOSD/EDR/Third_Party/nvcomp_results.csv"
...
echo "Running nvcomp benchmark on ${#DATASETS[@]} datasets..."
echo "Results will be saved incrementally to $OUTPUT"
...
for dataset in "${DATASETS[@]}"; do
  ...
  if [ -f "$filepath" ]; then
    ...
    ./benchmark_nvcomp -o "$OUTPUT" -f "$filepath" -a all -n 1 -w 1 -g 1
    # 显示当前结果行数
    lines=$(wc -l < "$OUTPUT")
    echo "  Current results: $lines lines in $OUTPUT"
  else
