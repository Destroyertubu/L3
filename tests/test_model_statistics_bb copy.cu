...
constexpr int WARP_SIZE_V4 = LANES_PER_MINI_VECTOR;  // Use configurable lane count
    ...
    T* __restrict__ output)
{
    ...
    // Warp mapping
    // ---------------------------------------------------------------------
    const int lane_id = threadIdx.x % WARP_SIZE_V4;
    const int warp_id_in_block = threadIdx.x / WARP_SIZE_V4;
    const int warps_per_block  = blockDim.x / WARP_SIZE_V4;
        ...
        if (delta_bits == 0) {
            ...
            if (delta_bits <= 32) {
                ...
                if (model_type == MODEL_LINEAR) {
                    ...
                    // FOR with delta_bits > 32: use 4-value unrolling for better ILP
                    #pragma unroll
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        uint64_t bit_pos0 = lane_bit_start + static_cast<uint64_t>(v) * delta_bits;
                        uint64_t bit_pos1 = bit_pos0 + delta_bits;
                        uint64_t bit_pos2 = bit_pos1 + delta_bits;
                        uint64_t bit_pos3 = bit_pos2 + delta_bits;

                        uint64_t d0 = Vertical::extract_branchless_64_rt(interleaved_array, bit_pos0, delta_bits);
                        uint64_t d1 = Vertical::extract_branchless_64_rt(interleaved_array, bit_pos1, delta_bits);
                        uint64_t d2 = Vertical::extract_branchless_64_rt(interleaved_array, bit_pos2, delta_bits);
                        uint64_t d3 = Vertical::extract_branchless_64_rt(interleaved_array, bit_pos3, delta_bits);
...
// ============================================================================
// V6 Decoder: SoA Metadata + RLE Support (Best of V4 and V5-Opt)
// ============================================================================
// V6 combines:
// - SoA metadata layout from V4 (coalesced access, ~40% faster metadata load)
// - RLE/CONSTANT support from V5-Opt (full model support)
//
// Memory access pattern for metadata:
//   V4/V6 SoA:  32 blocks read d_delta_bits[pid..pid+31] → 1 coalesced 128B transaction
//   V5 AoS:     each block reads d_metadata_v5[pid] → 32 separate 64B transactions

/**
 * V6 Decoder Kernel: SoA metadata loading + RLE support + BitReader32 extraction
 */
template<typename T>
    ...
    T* __restrict__ output)
{
    ...
    // V6 OPTIMIZATION: SoA metadata loading (coalesced access like V4)
    const int partition_start = d_start_indices[pid];
    const int partition_end = d_end_indices[pid];
    const int partition_size = partition_end - partition_start;
    const int delta_bits = d_delta_bits[pid];
    ...
    // Parameter loading (lazy load like V4 - only load what's needed)
    double p0 = 0.0, p1 = 0.0, p2 = 0.0, p3 = 0.0;
    if (d_param_offsets == nullptr) {
        const int param_base = pid * 4;
        p0 = d_model_params[param_base + 0];
    ...
    // RLE/CONSTANT handling - MUST MATCH V4 FORMAT (LINEAR model for values and counts)
    if (model_type == MODEL_CONSTANT) {
        const int num_runs = static_cast<int>(p0);

        // Get base_value from params_arr[1]
        T rle_base_value;
        if constexpr (sizeof(T) == 8) {
            rle_base_value = static_cast<T>(__double_as_longlong(p1));
        } else {
        ...
        // Single run (traditional CONSTANT): all values are base_value
        // OPTIMIZATION D: Vectorized write for single-run case
        if (num_runs == 1) {
            if constexpr (sizeof(T) == 8) {
                // 64-bit: use uint2 to write 2 values at once (16 bytes)
                const int vec_count = partition_size / 2;
                const uint64_t val64 = static_cast<uint64_t>(rle_base_value);
                const uint2 vec = make_uint2(val64, val64);
                uint2* vec_out = reinterpret_cast<uint2*>(&output[partition_start]);
                for (int i = threadIdx.x; i < vec_count; i += blockDim.x) {
                    vec_out[i] = vec;
                }
                // Handle tail (odd element)
                if ((partition_size & 1) && threadIdx.x == 0) {
                    output[partition_start + partition_size - 1] = rle_base_value;
                }
            } else {
                // 32-bit: use uint4 to write 4 values at once (16 bytes)
                const int vec_count = partition_size / 4;
                const uint32_t val32 = static_cast<uint32_t>(rle_base_value);
                const uint4 vec = make_uint4(val32, val32, val32, val32);
                uint4* vec_out = reinterpret_cast<uint4*>(&output[partition_start]);
                for (int i = threadIdx.x; i < vec_count; i += blockDim.x) {
                    vec_out[i] = vec;
                }
                // Handle tail (remaining 0-3 elements)
                const int tail_start = vec_count * 4;
                for (int i = tail_start + threadIdx.x; i < partition_size; i += blockDim.x) {
                    output[partition_start + i] = rle_base_value;
                }
            }
            return;
        }
        ...
        // =====================================================================
        // OPTIMIZED RLE DECODE: Parallel run decode + per-run parallel expand
        // =====================================================================
        // Header format: [value_intercept(f), value_slope(f), count_intercept(f), count_slope(f), bits_info, num_runs]
        __shared__ uint32_t s_rle_data[64];  // 64 words = 256 bytes, optimal for most RLE
        __shared__ T s_run_values[512];
        __shared__ int s_run_counts[512];
        __shared__ int s_run_offsets[513];

        const int64_t rle_bit_base = interleaved_base << 5;
        const int64_t rle_first_word = rle_bit_base >> 5;

        // Single-phase load: Load 64 words (covers most RLE partitions)
        constexpr int RLE_LOAD_WORDS = 64;
        for (int w = threadIdx.x; w < RLE_LOAD_WORDS; w += blockDim.x) {
            s_rle_data[w] = __ldg(&interleaved_array[rle_first_word + w]);
        }
        __syncthreads();

        // Parse header from shared memory
        const float* fptr = reinterpret_cast<const float*>(s_rle_data);
        const float s_value_intercept = fptr[0];
        const float s_value_slope = fptr[1];
        const float s_count_intercept = fptr[2];
        const float s_count_slope = fptr[3];
        const uint32_t bits_info = s_rle_data[4];
        const int value_bits = bits_info & 0xFF;
        const int count_bits = (bits_info >> 8) & 0xFF;
        const int actual_num_runs = static_cast<int>(s_rle_data[5]);

        // OPTIMIZATION 1: Parallel run decode - each thread decodes one run
        const int64_t values_bit_base = 192;
        const int64_t counts_bit_base = 192 + static_cast<int64_t>(actual_num_runs) * value_bits;
        const float vi = s_value_intercept, vs = s_value_slope;
        const float ci = s_count_intercept, cs = s_count_slope;

        // Fast path for zero-bit case (no residuals needed)
        if (value_bits == 0 && count_bits == 0) {
            for (int r = threadIdx.x; r < actual_num_runs && r < 512; r += blockDim.x) {
                float value_pred = vi + vs * static_cast<float>(r);
                s_run_values[r] = rle_base_value + static_cast<T>(lrintf(value_pred));
                float count_pred = ci + cs * static_cast<float>(r);
                s_run_counts[r] = static_cast<int>(lrintf(count_pred));
            }
        } else {
            // Each thread decodes one or more runs (with residuals)
        for (int r = threadIdx.x; r < actual_num_runs && r < 512; r += blockDim.x) {
            // Read value residual
            int64_t value_residual = 0;
            if (value_bits > 0) {
                const int64_t bit_offset = values_bit_base + static_cast<int64_t>(r) * value_bits;
                const int word_idx = bit_offset >> 5;
                const int bit_in_word = bit_offset & 31;
                uint64_t combined = (word_idx + 1 < RLE_LOAD_WORDS) ?
                    ((static_cast<uint64_t>(s_rle_data[word_idx + 1]) << 32) | s_rle_data[word_idx]) :
                    s_rle_data[word_idx];
                uint64_t raw = (combined >> bit_in_word) & ((1ULL << value_bits) - 1);
                if (value_bits < 64 && (raw & (1ULL << (value_bits - 1)))) {
                    raw |= ~((1ULL << value_bits) - 1);
                }
                value_residual = static_cast<int64_t>(raw);
            }
            ...
            int64_t count_residual = 0;
            if (count_bits > 0) {
                const int64_t bit_offset = counts_bit_base + static_cast<int64_t>(r) * count_bits;
                const int word_idx = bit_offset >> 5;
                const int bit_in_word = bit_offset & 31;
                uint64_t combined = (word_idx + 1 < RLE_LOAD_WORDS) ?
                    ((static_cast<uint64_t>(s_rle_data[word_idx + 1]) << 32) | s_rle_data[word_idx]) :
                    s_rle_data[word_idx];
                uint64_t raw = (combined >> bit_in_word) & ((1ULL << count_bits) - 1);
                if (count_bits < 64 && (raw & (1ULL << (count_bits - 1)))) {
                    raw |= ~((1ULL << count_bits) - 1);
                }
                count_residual = static_cast<int64_t>(raw);
            }

            // Reconstruct value and count using LINEAR model (float precision for speed)
            float value_pred = vi + vs * static_cast<float>(r);
            int64_t value_delta = static_cast<int64_t>(lrintf(value_pred)) + value_residual;
            s_run_values[r] = rle_base_value + static_cast<T>(value_delta);

            float count_pred = ci + cs * static_cast<float>(r);
            s_run_counts[r] = static_cast<int>(lrintf(count_pred)) + static_cast<int>(count_residual);
        }
        }  // End of else block for residuals case
        __syncthreads();

        // OPTIMIZATION 2: Parallel prefix sum for offsets (use first warp)
        if (threadIdx.x < 32) {
            // Warp 0 computes prefix sum
            ...
            for (int base = 0; base < actual_num_runs; base += 32) {
                ...
                // Write offset (exclusive prefix sum = inclusive - my_count + running_offset)
                if (r < actual_num_runs) {
                    s_run_offsets[r] = running_offset + my_count - s_run_counts[r];
                }

                // Update running offset with the last element's inclusive sum
        ...
        // OPTIMIZATION 3: Per-run parallel expand with warp cooperation
        // Each warp processes one run at a time - coalesced writes for good memory bandwidth
        const int warp_id = threadIdx.x / 32;
        const int lane_id = threadIdx.x & 31;
        const int num_warps = blockDim.x / 32;

        for (int r = warp_id; r < actual_num_runs; r += num_warps) {
            const int run_start = s_run_offsets[r];
            const int run_end = s_run_offsets[r + 1];
            const T run_value = s_run_values[r];

            // Warp cooperatively writes all values in this run (coalesced)
    ...
    // Process mini-vectors using BitReader32 (same as V4/V5-Opt)
    for (int mv_idx = warp_id_in_block; mv_idx < num_mv; mv_idx += warps_per_block) {
        const int mv_start_global = partition_start + mv_idx * MINI_VECTOR_SIZE;
        int out = mv_start_global + lane_id;
            ...
            if (delta_bits <= 32) {
                ...
                if (model_type == MODEL_FOR_BITPACK) {
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        ...
                        output[out + 0 * WARP_SIZE_V4] = applyDeltaV4(static_cast<T>(__double2ll_rn(y_fp)), dd0);
                        y_fp = FiniteDiff::d_add(y_fp, d1_fp); d1_fp = FiniteDiff::d_add(d1_fp, d2_fp);
                        output[out + 1 * WARP_SIZE_V4] = applyDeltaV4(static_cast<T>(__double2ll_rn(y_fp)), dd1);
                        y_fp = FiniteDiff::d_add(y_fp, d1_fp); d1_fp = FiniteDiff::d_add(d1_fp, d2_fp);
                        output[out + 2 * WARP_SIZE_V4] = applyDeltaV4(static_cast<T>(__double2ll_rn(y_fp)), dd2);
                        y_fp = FiniteDiff::d_add(y_fp, d1_fp); d1_fp = FiniteDiff::d_add(d1_fp, d2_fp);
                        output[out + 3 * WARP_SIZE_V4] = applyDeltaV4(static_cast<T>(__double2ll_rn(y_fp)), dd3);
                        y_fp = FiniteDiff::d_add(y_fp, d1_fp); d1_fp = FiniteDiff::d_add(d1_fp, d2_fp);
                        out += 4 * WARP_SIZE_V4;
                    }
                } else if (model_type == MODEL_POLYNOMIAL3) {
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        ...
                        output[out + 0 * WARP_SIZE_V4] = applyDeltaV4(pred_const, dd0);
                        output[out + 1 * WARP_SIZE_V4] = applyDeltaV4(pred_const, dd1);
                        output[out + 2 * WARP_SIZE_V4] = applyDeltaV4(pred_const, dd2);
                        output[out + 3 * WARP_SIZE_V4] = applyDeltaV4(pred_const, dd3);
                        out += 4 * WARP_SIZE_V4;
                    }
                }
            } else {
    ...
    // ---------------------------------------------------------------------
    // Tail values (partition_size % MINI_VECTOR_SIZE) - V4-style compact structure
    // ---------------------------------------------------------------------
    const int tail_start = num_mv * MINI_VECTOR_SIZE;
    const uint64_t tail_bit_base =
        (static_cast<uint64_t>(interleaved_base) << 5) +
    ...
    if (model_type == MODEL_FOR_BITPACK) {
        ...
        // Prediction + delta tail - handle delta_bits inside each model branch
        if (model_type == MODEL_LINEAR) {
            for (int local_idx = tail_start + threadIdx.x; local_idx < partition_size; local_idx += blockDim.x) {
                double x_tail = static_cast<double>(local_idx);
                double predicted = p0 + p1 * x_tail;
...
// V6 Wrapper: SoA metadata with RLE support
template<typename T>
void decompressV6(
    const CompressedDataVertical<T>& compressed,
    T* d_output,
    ...
    int block_size = 256)
{
    ...
    switch (block_size) {
        case 32:
            decompressInterleavedAllPartitionsV6<T><<<np, 32, 0, stream>>>(
                compressed.d_interleaved_deltas, compressed.d_start_indices,
                compressed.d_end_indices, compressed.d_model_types,
                compressed.d_model_params, compressed.d_param_offsets, compressed.d_delta_bits,
                compressed.d_num_mini_vectors, compressed.d_interleaved_offsets,
...
DatasetResult runModelStatistics(const DatasetInfo& dataset, int partition_size,
    ...
    int pure_constant_count = 0;   // MODEL_CONSTANT with delta_bits == 0
    int rle_count = 0;             // MODEL_CONSTANT with delta_bits > 0
    ...
    for (int i = 0; i < compressed.num_partitions; i++) {
        ...
        // Count pure CONSTANT vs RLE
        if (h_model_types[i] == MODEL_CONSTANT) {
            if (bits == 0) {
                pure_constant_count++;
            } else {
                rle_count++;
    ...
    std::cout << "  Pure CONSTANT (delta=0): " << pure_constant_count
              << " (" << std::fixed << std::setprecision(1) << (100.0 * pure_constant_count / compressed.num_partitions) << "%)\n";
    std::cout << "  RLE (CONSTANT, delta>0): " << rle_count
              << " (" << std::fixed << std::setprecision(1) << (100.0 * rle_count / compressed.num_partitions) << "%)\n";
    ...
    // TEMPORARILY DISABLED: V5-Opt has a bug with RLE partitions
    bool has_v5 = false; // compressed.use_v5_metadata && compressed.d_metadata_v5 != nullptr;
    ...
    // ========== V6 Decoder Timing (SoA + RLE) ==========
    double v6_time_ms = 0.0;
    double v6_gbps = 0.0;
    {
        std::cout << "  Testing V6 decoder (SoA+RLE)...\n";
        for (int i = 0; i < warmup; i++) {
            V4_decoder::decompressV6<T>(compressed, d_output, 0, actual_block_size);
            CUDA_CHECK(cudaDeviceSynchronize());
        }

        CudaTimer timer_v6;
        double total_decode_ms_v6 = 0.0;
        for (int i = 0; i < runs; i++) {
            timer_v6.start();
            V4_decoder::decompressV6<T>(compressed, d_output, 0, actual_block_size);
            total_decode_ms_v6 += timer_v6.stop();
        }
        v6_time_ms = total_decode_ms_v6 / runs;
        v6_gbps = data_gb / (v6_time_ms / 1000.0);

        // Use V6 result as primary (best of both worlds)
        result.decode_time_ms = v6_time_ms;
        result.decode_gbps = v6_gbps;
        std::cout << "    V6: " << std::fixed << std::setprecision(2) << v6_gbps << " GB/s\n";
    }
    ...
    // Print comparison (V6 is always available, it uses SoA arrays)
    std::cout << "    Speedup (V6 vs V4): " << std::fixed << std::setprecision(2) << (v6_gbps / v4_gbps) << "x\n";
    if (has_v5) {
        std::cout << "    Speedup (V6 vs V5-Opt): " << std::fixed << std::setprecision(2) << (v6_gbps / v5opt_gbps) << "x\n";
    }
...
DatasetResult processDataset(const DatasetInfo& dataset, int partition_size,
    ...
    DatasetResult result;
    if (dataset.is_signed) {
        result = runModelStatistics<int32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, sort_data, enable_rle);
    } else if (dataset.is_uint64) {
        result = runModelStatistics<uint64_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, sort_data, enable_rle);
    } else {
        result = runModelStatistics<uint32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, sort_data, enable_rle);
    }
    return result;
}
...
void printUsage(const char* program_name) {
    std::cerr << "Usage: " << program_name << " [OPTIONS]\n"
              ...
              << "  --no-rle                Disable RLE/CONSTANT model selection\n"
...
int main(int argc, char** argv) {
    ...
    bool enable_rle = true;    // Enable RLE/CONSTANT model (default: true)
    bool verbose = false;
    ...
    if (!skip_synthetic && single_dataset < 0) {
        ...
        for (const auto& dataset : SYNTHETIC_DATASETS) {
            ...
            if (format == "gm") {
                ...
                result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, sort_data, enable_rle);
            }
    ...
    if (scan_encoder_block_size && format == "vertical") {
        ...
        for (const auto& dataset : DATASETS) {
            ...
            for (int bs : encoder_block_sizes_to_test) {
                DatasetResult result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, bs, sort_data, enable_rle);
            ...
            DatasetResult best_result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, best.best_block_size, sort_data, enable_rle);
        ...
        if (!custom_file.empty()) {
            ...
            if (format == "gm") {
                ...
                result = processDataset(custom_dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, sort_data, enable_rle);
            }
