    ...
    T* __restrict__ output)
{
    ...
    // ---------------------------------------------------------------------
    // Lazy / minimal parameter loading (keeps register + memory traffic down)
    // ---------------------------------------------------------------------
    const int param_base = pid * 4;
    const double p0 = d_model_params[param_base + 0];
    ...
    // Base value for FOR_BITPACK (stored as raw int64 bits for 64-bit types)
    T base_value = T(0);
    ...
    for (int mv_idx = warp_id_in_block; mv_idx < num_mv; mv_idx += warps_per_block) {
        ...
        if (delta_bits == 0) {
            ...
            if (model_type == MODEL_FOR_BITPACK) {
                ...
                for (int v = 0; v < VALUES_PER_THREAD; ++v) {
                    double predicted = p0 + p1 * x;
                    output[out] = fp64_to_int<T>(predicted);
                    out += WARP_SIZE_V4;
                    x += X_STEP;
                }
            } else if (model_type == MODEL_POLYNOMIAL2) {
            ...
            if (delta_bits <= 32) {
                ...
                if (model_type == MODEL_FOR_BITPACK) {
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        ...
                        double pred0 = p0 + p1 * x;
                        T pv0 = fp64_to_int<T>(pred0);
                        output[out + 0 * WARP_SIZE_V4] = applyDeltaV4(pv0, d0);
                        x += X_STEP;

                        double pred1 = p0 + p1 * x;
                        T pv1 = fp64_to_int<T>(pred1);
                        output[out + 1 * WARP_SIZE_V4] = applyDeltaV4(pv1, d1);
                        x += X_STEP;

                        double pred2 = p0 + p1 * x;
                        T pv2 = fp64_to_int<T>(pred2);
                        output[out + 2 * WARP_SIZE_V4] = applyDeltaV4(pv2, d2);
                        x += X_STEP;

                        double pred3 = p0 + p1 * x;
                        T pv3 = fp64_to_int<T>(pred3);
                        output[out + 3 * WARP_SIZE_V4] = applyDeltaV4(pv3, d3);
                        x += X_STEP;
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        ...
                        double pred0 = p0 + x * (p1 + x * p2);
                        T pv0 = fp64_to_int<T>(pred0);
                        output[out + 0 * WARP_SIZE_V4] = applyDeltaV4(pv0, d0);
                        x += X_STEP;

                        double pred1 = p0 + x * (p1 + x * p2);
                        T pv1 = fp64_to_int<T>(pred1);
                        output[out + 1 * WARP_SIZE_V4] = applyDeltaV4(pv1, d1);
                        x += X_STEP;

                        double pred2 = p0 + x * (p1 + x * p2);
                        T pv2 = fp64_to_int<T>(pred2);
                        output[out + 2 * WARP_SIZE_V4] = applyDeltaV4(pv2, d2);
                        x += X_STEP;

                        double pred3 = p0 + x * (p1 + x * p2);
                        T pv3 = fp64_to_int<T>(pred3);
                        output[out + 3 * WARP_SIZE_V4] = applyDeltaV4(pv3, d3);
                        x += X_STEP;
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; v += 4) {
                        ...
                        double pred0 = p0 + x * (p1 + x * (p2 + x * p3));
                        T pv0 = fp64_to_int<T>(pred0);
                        output[out + 0 * WARP_SIZE_V4] = applyDeltaV4(pv0, d0);
                        x += X_STEP;

                        double pred1 = p0 + x * (p1 + x * (p2 + x * p3));
                        T pv1 = fp64_to_int<T>(pred1);
                        output[out + 1 * WARP_SIZE_V4] = applyDeltaV4(pv1, d1);
                        x += X_STEP;

                        double pred2 = p0 + x * (p1 + x * (p2 + x * p3));
                        T pv2 = fp64_to_int<T>(pred2);
                        output[out + 2 * WARP_SIZE_V4] = applyDeltaV4(pv2, d2);
                        x += X_STEP;

                        double pred3 = p0 + x * (p1 + x * (p2 + x * p3));
                        T pv3 = fp64_to_int<T>(pred3);
                        output[out + 3 * WARP_SIZE_V4] = applyDeltaV4(pv3, d3);
                        x += X_STEP;
                ...
                if (model_type == MODEL_FOR_BITPACK) {
                    ...
                    for (int v = 0; v < VALUES_PER_THREAD; ++v) {
                        ...
                        output[out] = applyDeltaV4(pv, delta);
    ...
    uint32_t* __restrict__ output)
{
    ...
    // Load base value (FOR only needs params[0])
...
DatasetResult runModelStatistics(const DatasetInfo& dataset, int partition_size,
                                  ...
                                  int encoder_block_size = 256, bool enable_sort = false) {
    ...
    // Sort data if enabled (ascending order)
    if (enable_sort) {
    ...
    // ========================================================================
    // Decoding with V4 decoder and timing
    // ========================================================================
    ...
    //     cudaMemcpy(h_model_params.data(), compressed.d_model_params, compressed.num_partitions * 4 * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    //     std::cerr << "    Params: theta0=" << std::scientific << std::setprecision(17) << h_model_params[0]
    //               << ", theta1=" << h_model_params[1] << "\n";
    //     std::cerr << "    Delta bits: " << h_delta_bits[0] << "\n";
    //     std::cerr << "    Range: [" << h_starts[0] << ", " << h_ends[0] << ")\n";
    //
    //     // Test GPU rounding
    //     std::cerr << "\n    GPU rounding test:\n";
    //     testGPURounding<<<1, 1>>>(h_model_params[0], h_model_params[1], 10);
    ...
    //         double x = static_cast<double>(i);
    //         double predicted = h_model_params[0] + h_model_params[1] * x;
...
DatasetResult processDataset(const DatasetInfo& dataset, int partition_size,
    ...
    DatasetResult result;
    if (dataset.is_signed) {
        result = runModelStatistics<int32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, enable_sort);
    } else if (dataset.is_uint64) {
        result = runModelStatistics<uint64_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, enable_sort);
    } else {
        result = runModelStatistics<uint32_t>(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, enable_sort);
    }
    return result;
}
...
void printUsage(const char* program_name) {
    std::cerr << "Usage: " << program_name << " [OPTIONS]\n"
              ...
              << "  --sort                  Sort data before compression (ascending order)\n"
...
int main(int argc, char** argv) {
    ...
    bool enable_sort = false;         // Sort data before compression
    ...
    if (enable_sort) {
        std::cout << "  Data Sorting: ENABLED (ascending order)\n";
    }
    std::cout << "\n";
    ...
    if (!skip_synthetic && single_dataset < 0) {
        ...
        for (const auto& dataset : SYNTHETIC_DATASETS) {
            ...
            if (format == "gm") {
                ...
                result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, enable_sort);
            }
    ...
    if (scan_encoder_block_size && format == "vertical") {
        ...
        std::vector<BestParams> all_best_params;
        ...
        for (const auto& dataset : DATASETS) {
            ...
            for (int bs : encoder_block_sizes_to_test) {
                DatasetResult result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, bs, enable_sort);
            ...
            all_best_params.push_back(best);

            // Store the best result for CSV output
            DatasetResult best_result = processDataset(dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, best.best_block_size, enable_sort);
        ...
        for (const auto& bp : all_best_params) {
        ...
        if (!custom_file.empty()) {
            ...
            if (format == "gm") {
                ...
                result = processDataset(custom_dataset, partition_size, min_partition, max_partition, enable_merging, warmup, runs, encoder_block_size, enable_sort);
            }
