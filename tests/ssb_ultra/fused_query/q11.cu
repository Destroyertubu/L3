/**
 * @file q11.cu
 * @brief SSB Q1.1 Implementation - Fused Query Strategy
 *
 * Query:
 ...
 *
 * Strategy: Decompress and process data partition-by-partition (streaming),
 *           avoiding full materialization of all columns.
 */
...
// Process chunk size - number of elements per streaming iteration
constexpr int CHUNK_SIZE = 1024 * 1024;  // 1M elements per chunk
    ...
    unsigned long long* revenue)
{
    ...
    // Load and filter orderdate (year 1993)
...
void runQ11Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;

    // Allocate chunk buffers (much smaller than full columns)
    ...
    // Process data in chunks - fused decompression and query
    ...
    for (int offset = 0; offset < total_elements; offset += CHUNK_SIZE) {
        ...
        q11_fused_kernel<<<grid_size, BLOCK_THREADS>>>(
    ...
    timing.data_load_ms = 0;  // Fused - no separate decompression time
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.1 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ11Fused(data, timing);
