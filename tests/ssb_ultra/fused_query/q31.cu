/**
 * @file q31.cu
 * @brief SSB Q3.1 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q31_fused_kernel(
    ...
    unsigned long long* agg_revenue)
{
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        ...
        // Probe customer (ASIA, get nation)
        ...
        // Probe supplier (ASIA, get nation)
...
__global__ void build_supplier_region_ht(const uint32_t* s_suppkey, const uint32_t* s_region, const uint32_t* s_nation,
                                          int n, uint32_t filter_region, uint32_t* ht_k, uint32_t* ht_v, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
...
void runQ31Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    build_customer_region_ht<<<(C_LEN+bs-1)/bs, bs>>>(data.d_c_custkey, data.d_c_region, data.d_c_nation, C_LEN, C_REGION_ASIA, ht_c_keys, ht_c_values, ht_c_size);
    build_supplier_region_ht<<<(S_LEN+bs-1)/bs, bs>>>(data.d_s_suppkey, data.d_s_region, data.d_s_nation, S_LEN, S_REGION_ASIA, ht_s_keys, ht_s_values, ht_s_size);
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q31_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_revenue, chunk_size,
    ...
    cudaFree(ht_d_keys); cudaFree(ht_d_values); cudaFree(ht_c_keys); cudaFree(ht_c_values);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.1 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ31Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ31Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
