/**
 * @file q33.cu
 * @brief SSB Q3.3 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q33_fused_kernel(
...
void runQ33Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q33_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_revenue, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.3 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ33Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ33Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
