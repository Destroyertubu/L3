/**
 * @file q32.cu
 * @brief SSB Q3.2 Implementation - Fused Query Strategy
 *
 * Query:
 *   SELECT c_city, s_city, d_year, SUM(lo_revenue) AS revenue
 *   FROM customer, lineorder, supplier, date
 *   WHERE lo_custkey = c_custkey AND lo_suppkey = s_suppkey AND lo_orderdate = d_datekey
 *     AND c_nation = 'UNITED STATES' AND s_nation = 'UNITED STATES' AND d_year >= 1992 AND d_year <= 1997
...
__global__ void q32_fused_kernel(
...
__global__ void build_customer_nation_city_ht(const uint32_t* c_custkey, const uint32_t* c_nation, const uint32_t* c_city,
...
void runQ32Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    build_customer_nation_city_ht<<<(C_LEN+bs-1)/bs, bs>>>(data.d_c_custkey, data.d_c_nation, data.d_c_city, C_LEN, C_NATION_US, ht_c_keys, ht_c_values, ht_c_size);
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q32_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_revenue, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.2 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ32Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ32Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
