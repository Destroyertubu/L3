/**
 * @file q43.cu
 * @brief SSB Q4.3 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q43_fused_kernel(
...
void runQ43Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q43_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_suppkey, d_partkey, d_revenue, d_supplycost, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.3 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ43Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ43Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
