/**
 * @file q22.cu
 * @brief SSB Q2.2 Implementation - Fused Query Strategy
 *
 * Query:
 *   SELECT SUM(lo_revenue), d_year, p_brand1
 *   FROM lineorder, date, part, supplier
 *   WHERE lo_orderdate = d_datekey AND lo_partkey = p_partkey AND lo_suppkey = s_suppkey
 *     AND p_brand1 >= 'MFGR#2221' AND p_brand1 <= 'MFGR#2228' AND s_region = 'ASIA'
...
__global__ void q22_fused_kernel(
...
void runQ22Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    for (int offset = 0; offset < LO_LEN; offset += CHUNK_SIZE) {
        ...
        q22_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_partkey, d_suppkey, d_revenue, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.2 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ22Fused(data, timing);

    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ22Fused(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
