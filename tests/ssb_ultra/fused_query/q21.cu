/**
 * @file q21.cu
 * @brief SSB Q2.1 Implementation - Fused Query Strategy
 *
 * Query:
...
__global__ void q21_fused_kernel(
...
void runQ21Fused(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;

    // Build hash tables for dimension tables
    timer.start();
    ...
    // Allocate chunk buffers and result
    ...
    for (int offset = 0; offset < total_elements; offset += CHUNK_SIZE) {
        ...
        q21_fused_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_partkey, d_suppkey, d_revenue, chunk_size,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.1 - Fused Query Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ21Fused(data, timing);
