/**
 * @file q21_fused_v7.cu
 * @brief SSB Q2.1 with Crystal-Style Perfect Hash (Vertical Compatible)
 *
 * V7 Key Change: Use Vertical-style hash tables with SINGLE memory access per probe.
 *
 * Root cause of V5's 5x performance gap:
 * - V5 uses MurmurHash3 + 32-iteration linear probing (up to 32 memory accesses per probe)
 * - Vertical uses perfect hash: (key - min) % len (SINGLE memory access)
 *
 * Expected speedup: ~4x (from 4.38ms to ~1.1ms)
 *
 * Query:
...
// Filter constants
constexpr uint32_t P_CATEGORY_MFGR12 = 12;  // MFGR#12 encoded value
constexpr uint32_t S_REGION_AMERICA = 1;     // AMERICA region code

// ============================================================================
// Main Fused Kernel V7 - Q2.1 with Crystal-Style Hash
// ============================================================================

__global__ void q21_fused_v7_kernel(
    // Compressed columns
    ...
    int num_tiles,
    int total_rows,
    // Hash tables (Crystal-style: single-probe, no collision handling)
    const uint32_t* __restrict__ ht_s,     // Supplier: keys only
    int ht_s_len,                           // S_LEN
    const uint32_t* __restrict__ ht_p,     // Part: interleaved [key,brand,key,brand,...]
    int ht_p_len,                           // P_LEN
    const uint32_t* __restrict__ ht_d,     // Date: interleaved [datekey,year,...]
    int ht_d_len,                           // DATE_HT_LEN (60130)
    // Output
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int mv = 0; mv < 4; mv++) {
        ...
        // V7: Crystal-style probe - SINGLE memory access, no collision handling
        // Vertical: slot = suppkey % ht_s_len, check ht_s[slot] != 0
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v]) {
        ...
        // V7: Crystal-style probe with value retrieval
        // Vertical: slot = partkey % ht_p_len, read [key,value] as uint64_t
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v]) {
        ...
        // Vertical: slot = (datekey - 19920101) % ht_d_len
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v]) {
...
void runQ21FusedV7(SSBDataCompressedVertical& data) {
    ...
    std::cout << "Hash table sizes: Date=" << DATE_HT_LEN << ", Part=" << P_LEN << ", Supplier=" << S_LEN << std::endl;

    // Warmup
    for (int i = 0; i < 5; i++) {
        cudaMemset(d_agg_revenue, 0, agg_size * sizeof(unsigned long long));
        q21_fused_v7_kernel<<<num_tiles, WARP_SZ>>>(
    ...
    std::cout << "Average kernel (V7): " << avg_time << " ms" << std::endl;
    std::cout << "V5 baseline: 4.38 ms" << std::endl;
    std::cout << "Vertical target: 0.89 ms" << std::endl;
    std::cout << "Speedup vs V5: " << (4.38 / avg_time) << "x" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q2.1 - V7 Crystal-Style Perfect Hash" << std::endl;
    std::cout << "======================================================" << std::endl;
    std::cout << "Key optimization: Vertical-compatible single-probe hash" << std::endl;
    std::cout << "  - V5: MurmurHash3 + 32-iteration linear probe" << std::endl;
    std::cout << "  - V7: (key - min) % len, single memory access" << std::endl;
    std::cout << std::endl;
