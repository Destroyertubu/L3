/**
 * @file q11_fused_v10.cu
 * @brief SSB Q1.1 - V10 Compact Metadata + 4x Parallelism
 *
 * Q1.1: Simple aggregation query (no joins)
...
// ============================================================================
// V10 Kernel - Q1.1 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q11_fused_v10_kernel(
    ...
    // Compact metadata
    const V10TileMetaQ11* __restrict__ tile_meta,
    int num_partitions,
    int total_rows,
    // Output
    unsigned long long* __restrict__ global_revenue)
{
    ...
    V10TileMetaQ11 meta = tile_meta[partition_id];
    ...
    // ========== Column 1: Orderdate (filter: year 1993) ==========
    ...
    for (int v = 0; v < 8; v++) {
        ...
        if (global_idx >= valid_in_tile) {
            flags[v] = 0;
        } else if (orderdate[v] < YEAR_1993_START || orderdate[v] > YEAR_1993_END) {
            flags[v] = 0;
        }
    }
    ...
    // ========== Column 3: Discount (filter: 1-3, compute) ==========
    uint32_t discount[8];
    {
...
// ============================================================================
// V10 Metadata Builder for Q1.1
// ============================================================================

std::vector<V10TileMetaQ11> buildV10MetadataQ11(
    ...
    int total_rows)
{
    ...
    std::vector<V10TileMetaQ11> meta(num_partitions);
...
void runQ11FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ11(
        data.lo_orderdate, data.lo_quantity, data.lo_discount,
        data.lo_extendedprice, total_rows);

    V10TileMetaQ11* d_meta;
    cudaMalloc(&d_meta, h_meta.size() * sizeof(V10TileMetaQ11));
    cudaMemcpy(d_meta, h_meta.data(), h_meta.size() * sizeof(V10TileMetaQ11), cudaMemcpyHostToDevice);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q11_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q1.1 V10 Verification ===" << std::endl;
    std::cout << "Revenue: " << h_revenue << std::endl;
    std::cout << "Expected: 40602899324" << std::endl;
    std::cout << "Status: " << (h_revenue == 40602899324ULL ? "PASSED" : "FAILED") << std::endl;
    ...
    std::cout << "V5 baseline: 0.71 ms" << std::endl;
    std::cout << "Vertical target: 0.544 ms" << std::endl;
    std::cout << "Speedup vs V5: " << (0.71 / avg_time) << "x" << std::endl;
    std::cout << "Gap to Vertical: " << (avg_time / 0.544) << "x" << std::endl;
...
int main(int argc, char** argv) {
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q1.1 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
