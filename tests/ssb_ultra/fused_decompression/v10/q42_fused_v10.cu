/**
 * @file q42_fused_v10.cu
 * @brief SSB Q4.2 with V10 Optimizations - Compact Metadata + 4x Parallelism
 *
...
// ============================================================================
// V10 Kernel - Q4.2 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q42_fused_v10_kernel(
    ...
    // Compact metadata
    const V10TileMetaQ42* __restrict__ tile_meta,
    int num_partitions,
    int total_rows,
    // Hash tables (Crystal-style)
    const uint32_t* __restrict__ ht_c,     // Customer: keys only
    int ht_c_len,
    const uint32_t* __restrict__ ht_s,     // Supplier: suppkey -> nation
    int ht_s_len,
    const uint32_t* __restrict__ ht_p,     // Part: partkey -> category
    int ht_p_len,
    ...
    long long* __restrict__ agg_profit)
{
    ...
    V10TileMetaQ42 meta = tile_meta[partition_idx];
    ...
    for (int v = 0; v < 8; v++) {
        if (flags[v]) {
            if (__ldg(&ht_c[custkey[v] % ht_c_len]) == 0) flags[v] = 0;
        }
    }
    ...
    // ========== Column 3: Partkey (get category) ==========
    uint32_t partkey[8], p_category[8];
    {
    ...
    for (int v = 0; v < 8; v++) {
        if (flags[v]) {
            ...
            if (kv != 0) {
                p_category[v] = static_cast<uint32_t>(kv >> 32);
            } else {
                flags[v] = 0;
            }
        }
    ...
    // ========== Aggregation: profit = revenue - supplycost, group by (year, s_nation, p_category) ==========
    #pragma unroll
    for (int v = 0; v < 8; v++) {
        if (flags[v]) {
...
// ============================================================================
// V10 Metadata Builder for Q4.2
// ============================================================================

std::vector<V10TileMetaQ42> buildV10MetadataQ42(
    ...
    int total_rows)
{
    ...
    std::vector<V10TileMetaQ42> meta(num_partitions);
...
void runQ42FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ42(
        data.lo_custkey, data.lo_suppkey, data.lo_partkey,
        data.lo_orderdate, data.lo_revenue, data.lo_supplycost, total_rows);

    V10TileMetaQ42* d_meta;
    cudaMalloc(&d_meta, h_meta.size() * sizeof(V10TileMetaQ42));
    cudaMemcpy(d_meta, h_meta.data(), h_meta.size() * sizeof(V10TileMetaQ42), cudaMemcpyHostToDevice);
    ...
    build_customer_ht_region_keys_crystal<<<(C_LEN + block_size - 1) / block_size, block_size>>>(
    ...
    build_part_ht_mfgr_category_crystal<<<(P_LEN + block_size - 1) / block_size, block_size>>>(
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q42_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q4.2 V10 Verification ===" << std::endl;
    int num_results = 0;
    long long total_profit = 0;
    for (int i = 0; i < agg_size; ++i) {
        if (h_agg[i] != 0) { num_results++; total_profit += h_agg[i]; }
    }
    std::cout << "Total groups: " << num_results << " (expected: ~50)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q4.2 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
