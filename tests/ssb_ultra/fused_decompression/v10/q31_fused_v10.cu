/**
 * @file q31_fused_v10.cu
 * @brief SSB Q3.1 with V10 Optimizations - Compact Metadata + 4x Parallelism
 *
 * V10 Key Optimizations (same as Q2.1 V10):
...
constexpr uint32_t REGION_ASIA = 2;  // ASIA region code
...
// ============================================================================
// V10 Kernel - Q3.1 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q31_fused_v10_kernel(
    // Compressed columns
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    // ========== Aggregation: (c_nation, s_nation, year) ==========
    #pragma unroll
    for (int v = 0; v < 8; v++) {
        if (flags[v]) {
...
// ============================================================================
// V10 Metadata Builder for Q3.1
// ============================================================================

std::vector<V10TileMeta> buildV10MetadataQ31(
...
void runQ31FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ31(
    ...
    build_customer_ht_region_crystal<<<(C_LEN + block_size - 1) / block_size, block_size>>>(
        ...
        REGION_ASIA, ht_customer.d_data, C_LEN);
    ...
    build_supplier_ht_region_nation_crystal<<<(S_LEN + block_size - 1) / block_size, block_size>>>(
        ...
        REGION_ASIA, ht_supplier.d_data, S_LEN);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q31_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q3.1 V10 Verification ===" << std::endl;
    int num_results = 0;
    ...
    std::cout << "V7 baseline: 2.07 ms" << std::endl;
    std::cout << "Vertical target: 2.02 ms" << std::endl;
    std::cout << "Speedup vs V7: " << (2.07 / avg_time) << "x" << std::endl;
    std::cout << "Gap to Vertical: " << (avg_time / 2.02) << "x" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q3.1 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
