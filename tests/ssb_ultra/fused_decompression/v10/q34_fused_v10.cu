/**
 * @file q34_fused_v10.cu
 * @brief SSB Q3.4 with V10 Optimizations - Compact Metadata + 4x Parallelism
 *
...
constexpr int AGG_NUM_YEARS = 1;  // Only 1997
...
// ============================================================================
// V10 Kernel - Q3.4 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q34_fused_v10_kernel(
    // Compressed columns
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    // ========== Column 3: Orderdate (get year, filtered by Dec1997) ==========
...
// ============================================================================
// V10 Metadata Builder for Q3.4
// ============================================================================

std::vector<V10TileMeta> buildV10MetadataQ34(
...
void runQ34FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ34(
    ...
    // Date: yearmonth = Dec1997 (19971201-19971231)
    CrystalHashTable ht_date;
    ht_date.allocate(DATE_HT_LEN, true);
    build_date_ht_yearmonth_crystal<<<(D_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_d_datekey, data.d_d_year, D_LEN,
        19971201, 19971231, ht_date.d_data, DATE_HT_LEN);

    CrystalHashTable ht_customer;
    ht_customer.allocate(C_LEN, true);
    build_customer_ht_city_crystal<<<(C_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_c_custkey, data.d_c_city, C_LEN,
        CITY_UK1, CITY_UK5, ht_customer.d_data, C_LEN);

    CrystalHashTable ht_supplier;
    ht_supplier.allocate(S_LEN, true);
    build_supplier_ht_city_crystal<<<(S_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_s_suppkey, data.d_s_city, S_LEN,
        CITY_UK1, CITY_UK5, ht_supplier.d_data, S_LEN);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q34_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q3.4 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
