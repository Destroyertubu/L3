/**
 * @file q13_fused_v10.cu
 * @brief SSB Q1.3 - V10 Compact Metadata + 4x Parallelism
 *
 * Q1.3: Simple aggregation query (no joins)
 * SELECT SUM(lo_extendedprice * lo_discount) as revenue
 * FROM lineorder, date
 * WHERE lo_orderdate = d_datekey
 *   AND d_weeknuminyear = 6 AND d_year = 1994  (Feb 7-13, 1994)
 *   AND lo_discount BETWEEN 5 AND 7
 *   AND lo_quantity BETWEEN 26 AND 35
 *
 * Differences from Q1.1:
 *   - Q1.1: year 1993, discount 1-3, quantity < 25
 *   - Q1.3: Week 6 of 1994 (19940207-19940213), discount 5-7, quantity 26-35
 *
...
// Q1.3 filter constants (Week 6 of 1994: Feb 7-13)
constexpr uint32_t WEEK6_1994_START = 19940207;
constexpr uint32_t WEEK6_1994_END = 19940213;
...
// ============================================================================
// V10 Kernel - Q1.3 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q13_fused_v10_kernel(
    ...
    // Compact metadata
    const V10TileMetaQ13* __restrict__ tile_meta,
    int num_partitions,
    int total_rows,
    // Output
    unsigned long long* __restrict__ global_revenue)
{
    ...
    V10TileMetaQ13 meta = tile_meta[partition_id];
    ...
    // ========== Column 1: Orderdate (filter: Week 6 of 1994) ==========
    ...
    for (int v = 0; v < 8; v++) {
        ...
        if (global_idx >= valid_in_tile) {
            flags[v] = 0;
        } else if (orderdate[v] < WEEK6_1994_START || orderdate[v] > WEEK6_1994_END) {
            flags[v] = 0;
        }
    }
    ...
    // ========== Column 3: Discount (filter: 5-7, compute) ==========
    uint32_t discount[8];
    {
...
// ============================================================================
// V10 Metadata Builder for Q1.3
// ============================================================================

std::vector<V10TileMetaQ13> buildV10MetadataQ13(
    ...
    int total_rows)
{
    ...
    std::vector<V10TileMetaQ13> meta(num_partitions);
...
void runQ13FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ13(
        data.lo_orderdate, data.lo_quantity, data.lo_discount,
        data.lo_extendedprice, total_rows);

    V10TileMetaQ13* d_meta;
    cudaMalloc(&d_meta, h_meta.size() * sizeof(V10TileMetaQ13));
    cudaMemcpy(d_meta, h_meta.data(), h_meta.size() * sizeof(V10TileMetaQ13), cudaMemcpyHostToDevice);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q13_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
    ...
    const unsigned long long EXPECTED = 2357989176ULL;
    std::cout << "\n=== Q1.3 V10 Verification ===" << std::endl;
...
int main(int argc, char** argv) {
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q1.3 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
    std::cout << "Query: SELECT SUM(lo_extendedprice * lo_discount)" << std::endl;
    std::cout << "Filters:" << std::endl;
    std::cout << "  - d_weeknuminyear = 6, d_year = 1994 (Feb 7-13)" << std::endl;
    std::cout << "  - lo_discount BETWEEN 5 AND 7" << std::endl;
