/**
 * @file q33_fused_v10.cu
 * @brief SSB Q3.3 with V10 Optimizations - Compact Metadata + 4x Parallelism
 *
...
constexpr uint32_t CITY_UK1 = 231;  // UNITED KI1
constexpr uint32_t CITY_UK5 = 235;  // UNITED KI5
...
// ============================================================================
// V10 Kernel - Q3.3 with Compact Metadata + 4x Parallelism
// ============================================================================

__global__ void q33_fused_v10_kernel(
    // Compressed columns
...
// ============================================================================
// V10 Metadata Builder for Q3.3
// ============================================================================

std::vector<V10TileMeta> buildV10MetadataQ33(
...
void runQ33FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ33(
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q33_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q3.3 V10 Verification ===" << std::endl;
    int num_results = 0;
    unsigned long long total_revenue = 0;
    for (int i = 0; i < agg_size; ++i) {
        if (h_agg[i] > 0) { num_results++; total_revenue += h_agg[i]; }
    }
    std::cout << "Total groups: " << num_results << " (expected: ~20)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q3.3 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
