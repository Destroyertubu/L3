/**
 * @file q23_fused_v10.cu
 * @brief SSB Q2.3 with V10 - Template-Specialized + Compact Metadata
 *
 * Q2.3: Join query with aggregation
 * SELECT SUM(lo_revenue), d_year, p_brand1
 * FROM lineorder, date, part, supplier
 * WHERE lo_orderdate = d_datekey AND lo_partkey = p_partkey
 *   AND lo_suppkey = s_suppkey
 *   AND p_brand1 = 'MFGR#2239' (brand = 260)
 *   AND s_region = 'EUROPE'
 * GROUP BY d_year, p_brand1
 *
 * Differences from Q2.1:
 *   - Q2.1: p_category = 'MFGR#12', s_region = 'AMERICA'
 *   - Q2.3: p_brand1 = 260 (exact), s_region = 'EUROPE'
 */
...
// Q2.3 filter constants (from ssb_common.cuh)
...
__global__ void q23_fused_v10_kernel(
    ...
    const V10TileMetaQ23* __restrict__ tile_meta,
    int num_partitions,
    int total_rows,
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    V10TileMetaQ23 meta = tile_meta[partition_idx];
    ...
    // ========== Column 1: Suppkey (filter: EUROPE region) ==========
    uint32_t suppkey[8];
    {
    ...
    // ========== Column 2: Partkey (filter: brand = 260) ==========
...
// ============================================================================
// V10 Metadata Builder for Q2.3
// ============================================================================

std::vector<V10TileMetaQ23> buildV10MetadataQ23(
    ...
    int total_rows)
{
    ...
    std::vector<V10TileMetaQ23> meta(num_partitions);
...
void runQ23FusedV10(SSBDataCompressedVertical& data) {
    ...
    auto h_meta = buildV10MetadataQ23(
        data.lo_suppkey, data.lo_partkey, data.lo_orderdate, data.lo_revenue, total_rows);

    V10TileMetaQ23* d_meta;
    cudaMalloc(&d_meta, h_meta.size() * sizeof(V10TileMetaQ23));
    cudaMemcpy(d_meta, h_meta.data(), h_meta.size() * sizeof(V10TileMetaQ23), cudaMemcpyHostToDevice);
    ...
    // Part hash table: exact brand = 260
    CrystalHashTable ht_part;
    ht_part.allocate(P_LEN, true);
    build_part_ht_brand_exact_crystal<<<(P_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_p_partkey, data.d_p_brand1, P_LEN,
        TARGET_BRAND, ht_part.d_data, P_LEN);

    // Supplier: EUROPE region
    ...
    build_supplier_ht_region_crystal<<<(S_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_s_suppkey, data.d_s_region, S_LEN, S_REGION_EUROPE,
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q23_fused_v10_kernel<<<num_blocks, V10_BLOCK_THREADS>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q2.3 - V10 Compact Metadata + 4x Parallelism" << std::endl;
    std::cout << "======================================================" << std::endl;
    std::cout << "Query: SELECT SUM(lo_revenue), d_year, p_brand1" << std::endl;
    std::cout << "Filters:" << std::endl;
    std::cout << "  - p_brand1 = 'MFGR#2239' (brand = 260)" << std::endl;
    std::cout << "  - s_region = 'EUROPE'" << std::endl;
    std::cout << std::endl;
