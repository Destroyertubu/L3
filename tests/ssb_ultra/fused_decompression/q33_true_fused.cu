/**
 * @file q33_true_fused.cu
 * @brief SSB Q3.3 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
// ============================================================================
// Optimized Q3.3 Kernel with Thread Coarsening and Two-Level Hash
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q33_optimized_fused_kernel(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // Probe customer hash table (c_city = UK1 or UK5, get c_city)
        #pragma unroll
        ...
        // Probe supplier hash table (s_city = UK1 or UK5, get s_city)
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
...
void runQ33TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Step 3: Warmup run
    // -------------------------------------------------------------------------
    q33_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
    ...
    std::cout << "\n=== Q3.3 Results (Optimized True Fused) ===" << std::endl;
    int num_results = 0;
    ...
    timing.print("Q3.3 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.3 - Optimized True Fused Decompression ===" << std::endl;
