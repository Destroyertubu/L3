/**
 * @file q11_true_fused.cu
 * @brief SSB Q1.1 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
/**
 * @brief Optimized Q1.1 kernel with all optimizations
 *
 * Optimizations:
 * 1. Thread coarsening: Each thread processes 4 elements
 * 2. Shared memory: Partition metadata loaded once per block
 * 3. Selection flags: Avoids warp divergence from continue statements
 * 4. Warp shuffle reduction: Faster than shared memory tree
 * 5. Tile-based: Better L1 cache utilization
 */
__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q11_optimized_fused_kernel(
...
void runQ11TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Warmup run
    q11_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
        // Orderdate
    ...
    std::cout << "\n=== Q1.1 Results (Optimized True Fused) ===" << std::endl;
    std::cout << "Revenue: " << h_revenue << std::endl;
    std::cout << "Partitions: " << num_partitions << std::endl;
    timing.print("Q1.1 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.1 - Optimized True Fused Decompression ===" << std::endl;
