/**
 * @file q41_true_fused.cu
 * @brief SSB Q4.1 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
constexpr int AGG_SIZE = AGG_NUM_YEARS * AGG_NUM_NATIONS;

// ============================================================================
// Optimized Q4.1 Kernel with Thread Coarsening and Two-Level Hash
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q41_optimized_fused_kernel(
    ...
    long long* __restrict__ agg_profit)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // ---- Column 1: Partkey (filter MFGR#1 or MFGR#2) ----
        ...
        // ---- Column 2: Suppkey (filter s_region = AMERICA) ----
        ...
        // ---- Column 3: Custkey (get c_nation, c_region = AMERICA) ----
        decompressTileConditional<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_custkey, smem.columns[2], tile_start,
        ...
        decompressTileConditional<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_orderdate, smem.columns[3], tile_start,
        ...
        // ---- Aggregation by (year, c_nation) ----
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                int year_idx = year[i] - 1992;
                if (year_idx >= 0 && year_idx < AGG_NUM_YEARS && c_nation[i] < AGG_NUM_NATIONS) {
                    int agg_idx = year_idx * AGG_NUM_NATIONS + c_nation[i];
...
void runQ41TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Part hash table (MFGR#1 or MFGR#2, keys only)
    HashTable ht_part;
    ...
    // Supplier hash table (s_region = AMERICA, keys only)
    ...
    // Customer hash table (c_region = AMERICA, custkey -> nation)
    ...
    // Step 3: Warmup run
    // -------------------------------------------------------------------------
    q41_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
    ...
    std::cout << "\n=== Q4.1 Results (Optimized True Fused) ===" << std::endl;
    int num_results = 0;
    long long total_profit = 0;
    for (int y = 0; y < AGG_NUM_YEARS; ++y) {
        for (int n = 0; n < AGG_NUM_NATIONS; ++n) {
            int idx = y * AGG_NUM_NATIONS + n;
    ...
    timing.print("Q4.1 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.1 - Optimized True Fused Decompression ===" << std::endl;
