/**
 * @file q13_true_fused.cu
 * @brief SSB Q1.3 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
// ============================================================================
// Optimized Q1.3 Kernel with Thread Coarsening
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q13_optimized_fused_kernel(
    ...
    unsigned long long* __restrict__ revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // Q1.3 Predicate: Week 6 of 1994 (Feb 7-13, 1994: 19940207-19940213)
        BlockPredAndBetween<uint32_t, FUSED_ITEMS_PER_THREAD>(
            orderdate, 19940207u, 19940213u, selection_flags);
        ...
        // Q1.3 Predicate: discount BETWEEN 5 AND 7
        BlockPredAndBetween<uint32_t, FUSED_ITEMS_PER_THREAD>(
            discount, 5u, 7u, selection_flags);
        ...
        // Q1.3 Predicate: quantity BETWEEN 26 AND 35
        BlockPredAndBetween<uint32_t, FUSED_ITEMS_PER_THREAD>(
...
void runQ13TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Warmup run
    q13_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
        // Orderdate
    ...
    std::cout << "\n=== Q1.3 Results (Optimized True Fused) ===" << std::endl;
    std::cout << "Revenue: " << h_revenue << std::endl;
    std::cout << "Partitions: " << num_partitions << std::endl;
    timing.print("Q1.3 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.3 - Optimized True Fused Decompression ===" << std::endl;
