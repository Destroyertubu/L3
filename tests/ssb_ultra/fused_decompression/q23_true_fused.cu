/**
 * @file q23_true_fused.cu
 * @brief SSB Q2.3 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
// Filter constants
constexpr uint32_t P_BRAND_TARGET = 239;   // MFGR#2239 exact match
constexpr uint32_t S_REGION_EUROPE = 3;    // EUROPE region code

// Aggregation dimensions
constexpr int AGG_NUM_YEARS = 7;   // 1992-1998 (single brand, only year grouping)

// ============================================================================
// Optimized Q2.3 Kernel with Thread Coarsening and Two-Level Hash
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q23_optimized_fused_kernel(
    ...
    int ht_s_size,
    const uint32_t* __restrict__ ht_p_keys,  // Keys only for exact brand match
    int ht_p_size,
    const uint32_t* __restrict__ ht_d_keys,
    const uint32_t* __restrict__ ht_d_values,
    int ht_d_size,
    // Output: per-year aggregation (single brand)
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // Probe supplier hash table (two-level hash) - s_region = EUROPE
        #pragma unroll
        ...
        // ---- Column 2: Partkey (conditional, check exact brand) ----
        ...
        // Probe part hash table (two-level hash, exact brand match)
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                if (!ht_probe_exists_fast(ht_p_keys, ht_p_size, partkey[i])) {
                    selection_flags[i] = 0;
                }
            }
        }
        ...
        // ---- Aggregation by year only (single brand) ----
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
...
void runQ23TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Part hash table (exact brand match, keys only)
    HashTable ht_part;
    ht_part.allocate(P_LEN, false);
    int grid_p = (P_LEN + block_size - 1) / block_size;
    build_part_ht_brand_exact_kernel<<<grid_p, block_size>>>(
    ...
    // -------------------------------------------------------------------------
    // Step 2: Allocate aggregation arrays (per-year only)
    // -------------------------------------------------------------------------
    unsigned long long* d_agg_revenue;
    cudaMalloc(&d_agg_revenue, AGG_NUM_YEARS * sizeof(unsigned long long));
    cudaMemset(d_agg_revenue, 0, AGG_NUM_YEARS * sizeof(unsigned long long));

    // -------------------------------------------------------------------------
    // Step 3: Warmup run
    // -------------------------------------------------------------------------
    q23_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
        // Suppkey
    ...
    // -------------------------------------------------------------------------
    std::vector<unsigned long long> h_agg_revenue(AGG_NUM_YEARS);
    cudaMemcpy(h_agg_revenue.data(), d_agg_revenue, AGG_NUM_YEARS * sizeof(unsigned long long), cudaMemcpyDeviceToHost);

    std::cout << "\n=== Q2.3 Results (Optimized True Fused) ===" << std::endl;
    ...
    for (int y = 0; y < AGG_NUM_YEARS; ++y) {
        if (h_agg_revenue[y] > 0) {
            num_results++;
            total_revenue += h_agg_revenue[y];
            std::cout << (1992 + y) << "\t" << P_BRAND_TARGET << "\t" << h_agg_revenue[y] << std::endl;
        }
    }
    std::cout << "Total groups: " << num_results << ", Total revenue: " << total_revenue << std::endl;
    std::cout << "Partitions: " << num_partitions << std::endl;
    timing.print("Q2.3 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.3 - Optimized True Fused Decompression ===" << std::endl;
