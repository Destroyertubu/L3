/**
 * @file q31_true_fused.cu
 * @brief SSB Q3.1 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
constexpr int AGG_SIZE = AGG_NUM_YEARS * AGG_NUM_NATIONS * AGG_NUM_NATIONS;

// ============================================================================
// Optimized Q3.1 Kernel with Thread Coarsening and Two-Level Hash
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q31_optimized_fused_kernel(
    ...
    int ht_d_size,
    // Output: aggregation by (year, c_nation, s_nation)
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        uint32_t c_nation[FUSED_ITEMS_PER_THREAD];
        ...
        // ---- Column 1: Custkey (check customer filter first) ----
        decompressTileCoarsened<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_custkey, smem.columns[0], tile_start,
            custkey, selection_flags, partition_size);

        // Probe customer hash table (c_region = ASIA, get c_nation)
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                if (!ht_probe_get_fast(ht_c_keys, ht_c_values, ht_c_size, custkey[i], c_nation[i])) {
                    selection_flags[i] = 0;
                }
            }
        }
        ...
        // ---- Column 2: Suppkey (conditional, get s_nation) ----
        decompressTileConditional<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_suppkey, smem.columns[1], tile_start,
            suppkey, selection_flags, partition_size);

        // Probe supplier hash table (s_region = ASIA, get s_nation)
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
        ...
        // Probe date hash table and filter by year range
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                if (!ht_probe_get_fast(ht_d_keys, ht_d_values, ht_d_size, orderdate[i], year[i])) {
                    selection_flags[i] = 0;
                } else if (year[i] < 1992 || year[i] > 1997) {
                    selection_flags[i] = 0;
                }
            }
        }
        ...
        // ---- Aggregation by (year, c_nation, s_nation) ----
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                int year_idx = year[i] - 1992;
                if (year_idx >= 0 && year_idx < AGG_NUM_YEARS &&
                    c_nation[i] < AGG_NUM_NATIONS && s_nation[i] < AGG_NUM_NATIONS) {
                    int agg_idx = year_idx * AGG_NUM_NATIONS * AGG_NUM_NATIONS +
                                  c_nation[i] * AGG_NUM_NATIONS + s_nation[i];
...
void runQ31TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Customer hash table (c_region = ASIA, custkey -> nation)
    ...
    // Supplier hash table (s_region = ASIA, suppkey -> nation)
    ...
    // Step 3: Warmup run
    // -------------------------------------------------------------------------
    q31_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
    ...
    std::cout << "\n=== Q3.1 Results (Optimized True Fused) ===" << std::endl;
    int num_results = 0;
    unsigned long long total_revenue = 0;
    for (int y = 0; y < AGG_NUM_YEARS; ++y) {
        for (int cn = 0; cn < AGG_NUM_NATIONS; ++cn) {
            for (int sn = 0; sn < AGG_NUM_NATIONS; ++sn) {
                int idx = y * AGG_NUM_NATIONS * AGG_NUM_NATIONS + cn * AGG_NUM_NATIONS + sn;
    ...
    timing.print("Q3.1 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.1 - Optimized True Fused Decompression ===" << std::endl;
