/**
 * @file q11_true_fused_float.cu
 * @brief SSB Q1.1 Implementation - FLOAT-OPTIMIZED True Fused Decompression
 *
 * Query:
 ...
 * - FLOAT instead of DOUBLE for polynomial prediction (1.57x speedup)
...
#include "fused_decompress_tile_float.cuh"  // Float-optimized version
#include "l3_decompress_device.cuh"

using namespace ssb;
using namespace l3_fused;
using namespace l3_fused_float;

// ============================================================================
// Optimized Q1.1 Kernel with Thread Coarsening
// ============================================================================

/**
 * @brief FLOAT-Optimized Q1.1 kernel
 *
 * Optimizations:
 * 1. FLOAT polynomial prediction (not double) - 1.57x faster
 * 2. Thread coarsening: Each thread processes 4 elements
 * 3. Shared memory: Partition metadata loaded once per block
 * 4. Selection flags: Avoids warp divergence from continue statements
 * 5. Warp shuffle reduction: Faster than shared memory tree
 * 6. Tile-based: Better L1 cache utilization
 */
__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q11_float_fused_kernel(
    ...
    unsigned long long* __restrict__ revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // ---- Column 1: Orderdate ---- (using FLOAT decompression)
        decompressTileCoarsenedFloat<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_orderdate, smem.columns[0], tile_start,
            orderdate, selection_flags, partition_size);

        // Predicate: year 1993 (19930101 <= orderdate <= 19931231)
        BlockPredAndBetween<uint32_t, FUSED_ITEMS_PER_THREAD>(
            orderdate, 19930101u, 19931231u, selection_flags);

        // Early termination: skip remaining work if no rows pass
        if (l3_fused::IsTerm<FUSED_ITEMS_PER_THREAD>(selection_flags)) continue;

        // ---- Column 2: Discount (conditional on orderdate passing) ---- (FLOAT)
        decompressTileConditionalFloat<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_discount, smem.columns[1], tile_start,
            discount, selection_flags, partition_size);

        // Predicate: discount BETWEEN 1 AND 3
        BlockPredAndBetween<uint32_t, FUSED_ITEMS_PER_THREAD>(
            discount, 1u, 3u, selection_flags);

        // Early termination
        if (l3_fused::IsTerm<FUSED_ITEMS_PER_THREAD>(selection_flags)) continue;

        // ---- Column 3: Quantity (conditional on discount passing) ---- (FLOAT)
        decompressTileConditionalFloat<uint32_t, FUSED_ITEMS_PER_THREAD>(
            delta_array_quantity, smem.columns[2], tile_start,
            quantity, selection_flags, partition_size);

        // Predicate: quantity < 25
        BlockPredAndLT<uint32_t, FUSED_ITEMS_PER_THREAD>(
            quantity, 25u, selection_flags);

        // Early termination
        if (l3_fused::IsTerm<FUSED_ITEMS_PER_THREAD>(selection_flags)) continue;

        // ---- Column 4: Extendedprice (conditional on quantity passing) ---- (FLOAT)
        decompressTileConditionalFloat<uint32_t, FUSED_ITEMS_PER_THREAD>(
...
void runQ11TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Warmup run
    q11_float_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
        // Orderdate
    ...
    std::cout << "\n=== Q1.1 Results (Float-Optimized True Fused) ===" << std::endl;
    std::cout << "Revenue: " << h_revenue << std::endl;
    std::cout << "Partitions: " << num_partitions << std::endl;
    timing.print("Q1.1 Float-Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.1 - FLOAT-Optimized True Fused Decompression ===" << std::endl;
    std::cout << "Data directory: " << data_dir << std::endl;
    std::cout << "Float polynomial prediction (1.57x faster than double)" << std::endl;
