/**
 * @file q21_fused_v5.cu
 * @brief SSB Q2.1 with L3 Native Format + Prefetch Strategy
 *
 * Query:
 ...
 *
 * Columns: suppkey, partkey, orderdate, revenue
 * Joins: Supplier (region filter), Part (category filter, get brand), Date (get year)
 */
...
// ============================================================================
// Main Fused Kernel V5 - Q2.1
// ============================================================================

__global__ void q21_fused_v5_kernel(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int mv = 0; mv < 4; mv++) {
        ...
        // Probe supplier hash table (s_region = 'AMERICA')
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v]) {
        ...
        // Probe part hash table (p_category = 'MFGR#12', get brand)
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v]) {
                if (!ht_probe_get_fast(ht_p_keys, ht_p_values, ht_p_size, partkey[v], brand[v])) {
                    flags[v] = 0;
                }
            }
        }
...
void runQ21FusedV5(SSBDataCompressedVertical& data) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q21_fused_v5_kernel<<<num_tiles, WARP_SZ>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q2.1 - L3 Native Format + Prefetch Strategy (V5)" << std::endl;
    std::cout << "======================================================" << std::endl;
    std::cout << "Joins: Supplier(AMERICA), Part(MFGR#12), Date" << std::endl;
    std::cout << "Group by: year, brand" << std::endl;
    std::cout << std::endl;
