/**
 * @file q32_true_fused.cu
 * @brief SSB Q3.2 Implementation - OPTIMIZED True Fused Decompression
 *
 * Query:
...
constexpr int AGG_SIZE = AGG_NUM_YEARS * AGG_NUM_CITIES * AGG_NUM_CITIES;

// ============================================================================
// Optimized Q3.2 Kernel with Thread Coarsening and Two-Level Hash
// ============================================================================

__global__ __launch_bounds__(FUSED_BLOCK_SIZE, 4)
void q32_optimized_fused_kernel(
    ...
    int ht_d_size,
    // Output: aggregation by (year, c_city, s_city)
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int tile_start = 0; tile_start < partition_size; tile_start += FUSED_TILE_SIZE) {
        ...
        // Probe customer hash table (c_nation = US, get c_city)
        #pragma unroll
        ...
        // ---- Column 2: Suppkey (conditional, get s_city) ----
        ...
        // Probe supplier hash table (s_nation = US, get s_city)
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
        ...
        // ---- Aggregation by (year, c_city, s_city) ----
        #pragma unroll
        for (int i = 0; i < FUSED_ITEMS_PER_THREAD; ++i) {
            if (selection_flags[i]) {
                int year_idx = year[i] - 1992;
                if (year_idx >= 0 && year_idx < AGG_NUM_YEARS &&
                    c_city[i] < AGG_NUM_CITIES && s_city[i] < AGG_NUM_CITIES) {
                    int agg_idx = year_idx * AGG_NUM_CITIES * AGG_NUM_CITIES +
                                  c_city[i] * AGG_NUM_CITIES + s_city[i];
...
void runQ32TrueFused(SSBDataCompressed& data, QueryTiming& timing) {
    ...
    // Customer hash table (c_nation = US, custkey -> city)
    ...
    // Step 3: Warmup run
    // -------------------------------------------------------------------------
    q32_optimized_fused_kernel<<<num_partitions, FUSED_BLOCK_SIZE>>>(
    ...
    std::cout << "\n=== Q3.2 Results (Optimized True Fused) ===" << std::endl;
    int num_results = 0;
    ...
    for (int y = 0; y < AGG_NUM_YEARS; ++y) {
        for (int cc = 0; cc < AGG_NUM_CITIES; ++cc) {
            for (int sc = 0; sc < AGG_NUM_CITIES; ++sc) {
                int idx = y * AGG_NUM_CITIES * AGG_NUM_CITIES + cc * AGG_NUM_CITIES + sc;
    ...
    timing.print("Q3.2 Optimized True Fused");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.2 - Optimized True Fused Decompression ===" << std::endl;
