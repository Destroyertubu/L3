/**
 * @file q34_fused_v5.cu
 * @brief SSB Q3.4 with L3 Native Format + Prefetch Strategy
 *
 * Query:
...
__global__ void q34_fused_v5_kernel(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int mv = 0; mv < 4; mv++) {
        ...
        // Date filter: Dec 1997 (keys only, no value lookup)
        ...
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v] && !ht_probe_exists_fast(ht_d_keys, ht_d_size, orderdate[v])) flags[v] = 0;
        }
        if (__ballot_sync(0xFFFFFFFF, flags[0]|flags[1]|flags[2]|flags[3]|flags[4]|flags[5]|flags[6]|flags[7]) == 0) continue;
        ...
        // Aggregate by (c_city, s_city) - year is fixed (1997)
        #pragma unroll
        for (int v = 0; v < 8; v++) {
            if (flags[v] && c_city[v] < AGG_NUM_CITIES && s_city[v] < AGG_NUM_CITIES) {
                int agg_idx = c_city[v] * AGG_NUM_CITIES + s_city[v];
...
void runQ34FusedV5(SSBDataCompressedVertical& data) {
    ...
    build_customer_ht_city_filter_kernel<<<(C_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_c_custkey, data.d_c_city, C_LEN, CITY_UK1, CITY_UK5,
    ...
    build_supplier_ht_city_filter_kernel<<<(S_LEN + block_size - 1) / block_size, block_size>>>(
        data.d_s_suppkey, data.d_s_city, S_LEN, CITY_UK1, CITY_UK5,
        ht_supplier.d_keys, ht_supplier.d_values, ht_supplier.size);
    cudaDeviceSynchronize();

    int agg_size = AGG_NUM_CITIES * AGG_NUM_CITIES;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        q34_fused_v5_kernel<<<num_tiles, WARP_SZ>>>(
...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================" << std::endl;
    std::cout << "SSB Q3.4 - L3 Native Format + Prefetch Strategy (V5)" << std::endl;
    std::cout << "======================================================" << std::endl;
