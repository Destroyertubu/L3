/**
 * @file q34.cu
 * @brief SSB Q3.4 Implementation - Staged Random Access Optimization
 *
 * Query:
 ...
 *   Stage 1: Decompress suppkey, probe supplier city filter → ~0.8%
 *   Stage 2: Random access custkey for passing rows, probe customer filter
 *   Stage 3: Random access orderdate + revenue, aggregate with Dec 1997 filter
 */
...
constexpr int AGG_SIZE = NUM_CITIES * NUM_CITIES;

/**
 * @brief Stage 1: Probe supplier city filter
 */
__global__ void probeSupplierFilterQ34Kernel(
 ...
 */
__global__ void probeCustomerWithIndicesQ34Kernel(
...
__global__ void aggregateQ34Kernel(
    ...
    unsigned long long* __restrict__ agg_revenue)
{
    ...
    for (int i = tid; i < num_rows; i += blockDim.x * gridDim.x) {
        ...
        // Check if date is in Dec 1997
        int d_slot = hash_murmur3(od, ht_d_size);
        bool found = false;
        for (int probe = 0; probe < 32; ++probe) {
            int slot = (d_slot + probe) % ht_d_size;
            if (ht_d_keys[slot] == od) { found = true; break; }
            if (ht_d_keys[slot] == static_cast<uint32_t>(HT_EMPTY)) break;
        }

        if (found && c_city < NUM_CITIES && s_city < NUM_CITIES) {
            int agg_idx = c_city * NUM_CITIES + s_city;
            atomicAdd(&agg_revenue[agg_idx], static_cast<unsigned long long>(rev));
        }
    }
}

__global__ void build_date_dec1997_ht_q34(const uint32_t* d_datekey, int n, uint32_t start, uint32_t end, uint32_t* ht_k, int ht_size) {
...
__global__ void build_city_filter_ht_q34(const uint32_t* keys, const uint32_t* cities, int n,
                                          uint32_t city1, uint32_t city2, uint32_t* ht_k, uint32_t* ht_v, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    uint32_t city = cities[idx];
...
void runQ34RandomAccess(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    uint32_t *ht_d_keys, *ht_c_keys, *ht_c_values, *ht_s_keys, *ht_s_values;
    ...
    build_date_dec1997_ht_q34<<<(D_LEN+255)/256, 256>>>(data.d_d_datekey, D_LEN, DATE_DEC1997_START, DATE_DEC1997_END, ht_d_keys, ht_d_size);
    build_city_filter_ht_q34<<<(C_LEN+255)/256, 256>>>(data.d_c_custkey, data.d_c_city, C_LEN, CITY_UK1, CITY_UK5, ht_c_keys, ht_c_values, ht_c_size);
    build_city_filter_ht_q34<<<(S_LEN+255)/256, 256>>>(data.d_s_suppkey, data.d_s_city, S_LEN, CITY_UK1, CITY_UK5, ht_s_keys, ht_s_values, ht_s_size);
    ...
    probeSupplierFilterQ34Kernel<<<grid_stage1, block_size>>>(
    ...
    if (h_num_stage1 == 0) {
        std::cout << "\n=== Q3.4 Results (Staged Random Access) ===\nTotal: 0\n";
        cudaFree(d_stage1_indices); cudaFree(d_stage1_s_cities);
        cudaFree(ht_d_keys); cudaFree(ht_c_keys); cudaFree(ht_c_values); cudaFree(ht_s_keys); cudaFree(ht_s_values);
        return;
    }
    ...
    probeCustomerWithIndicesQ34Kernel<<<grid_stage2, block_size>>>(
    ...
    std::cout << "\n=== Q3.4 Results (Staged Random Access) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.4 - Staged Random Access ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);
    QueryTiming timing;
    runQ34RandomAccess(data, timing);
    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ34RandomAccess(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
