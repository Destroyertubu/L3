/**
 * @file q41.cu
 * @brief SSB Q4.1 Implementation - Predicate Pushdown Strategy
 */
...
__global__ void q41_predicate_kernel(
    const uint32_t* __restrict__ lo_orderdate, const uint32_t* __restrict__ lo_custkey,
    const uint32_t* __restrict__ lo_suppkey, const uint32_t* __restrict__ lo_partkey,
    const uint32_t* __restrict__ lo_revenue, const uint32_t* __restrict__ lo_supplycost,
    int num_rows,
    ...
    long long* agg_profit)
{
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        ...
        // Probe customer (AMERICA, get nation)
...
__global__ void build_date_ht(const uint32_t* d_datekey, const uint32_t* d_year, int n,
                               uint32_t* ht_k, uint32_t* ht_v, int ht_size) {
...
__global__ void build_customer_region_ht(const uint32_t* c_custkey, const uint32_t* c_region, const uint32_t* c_nation,
                                          int n, uint32_t filter_region, uint32_t* ht_k, uint32_t* ht_v, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x; if (idx >= n) return;
    if (c_region[idx] != filter_region) return;
    uint32_t key = c_custkey[idx], val = c_nation[idx];
    int slot = hash_murmur3(key, ht_size);
    for (int p = 0; p < ht_size; ++p) {
        int s = (slot + p) % ht_size;
        uint32_t old = atomicCAS(&ht_k[s], static_cast<uint32_t>(HT_EMPTY), key);
...
__global__ void build_supplier_region_ht(const uint32_t* s_suppkey, const uint32_t* s_region, int n,
...
__global__ void build_part_mfgr_ht(const uint32_t* p_partkey, const uint32_t* p_mfgr, int n,
                                    uint32_t mfgr1, uint32_t mfgr2, uint32_t* ht_k, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x; if (idx >= n) return;
...
void runQ41PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    int ht_d_size = D_LEN * 2, ht_c_size = C_LEN * 2, ht_s_size = S_LEN * 2, ht_p_size = P_LEN * 2;
    uint32_t *ht_d_keys, *ht_d_values, *ht_c_keys, *ht_c_values, *ht_s_keys, *ht_p_keys;
    ...
    build_customer_region_ht<<<(C_LEN+bs-1)/bs, bs>>>(data.d_c_custkey, data.d_c_region, data.d_c_nation, C_LEN, C_REGION_AMERICA, ht_c_keys, ht_c_values, ht_c_size);
    build_supplier_region_ht<<<(S_LEN+bs-1)/bs, bs>>>(data.d_s_suppkey, data.d_s_region, S_LEN, S_REGION_AMERICA, ht_s_keys, ht_s_size);
    build_part_mfgr_ht<<<(P_LEN+bs-1)/bs, bs>>>(data.d_p_partkey, data.d_p_mfgr, P_LEN, P_MFGR_1, P_MFGR_2, ht_p_keys, ht_p_size);
    ...
    cudaMalloc(&d_supplycost, LO_LEN * sizeof(uint32_t));
    ...
    acc_supplycost.decompressAll(d_supplycost);

    int grid_size = (LO_LEN + TILE_SIZE - 1) / TILE_SIZE;
    q41_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_custkey, d_suppkey, d_partkey, d_revenue, d_supplycost, LO_LEN,
        ht_d_keys, ht_d_values, ht_d_size, ht_c_keys, ht_c_values, ht_c_size, ht_s_keys, ht_s_size, ht_p_keys, ht_p_size, d_agg_profit);
    cudaDeviceSynchronize();
    timer.stop();
    ...
    std::cout << "\n=== Q4.1 Results (Predicate Pushdown) ===" << std::endl;
    ...
    cudaFree(ht_d_keys); cudaFree(ht_d_values); cudaFree(ht_c_keys); cudaFree(ht_c_values); cudaFree(ht_s_keys); cudaFree(ht_p_keys);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.1 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ41PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ41PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
