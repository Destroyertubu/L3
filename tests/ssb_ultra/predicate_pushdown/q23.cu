/**
 * @file q23.cu
 * @brief SSB Q2.3 Implementation - Predicate Pushdown Strategy
 */
...
__global__ void q23_predicate_kernel(
...
void runQ23PredicatePushdown(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    timer.start();
    ...
    q23_predicate_kernel<<<grid_size, BLOCK_THREADS>>>(d_orderdate, d_partkey, d_suppkey, d_revenue, LO_LEN,
    ...
    std::cout << "\n=== Q2.3 Results (Predicate Pushdown) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.3 - Predicate Pushdown Strategy ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ23PredicatePushdown(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ23PredicatePushdown(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
