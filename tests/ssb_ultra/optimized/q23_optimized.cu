/**
 * @file q23_optimized.cu
 * @brief SSB Q2.3 Implementation - Optimized with Two-Level Fast Hash
 * Query: s_region = 'EUROPE', p_brand = 'MFGR#2239'
 */
...
__global__ void probePartWithExactBrandQ23(
...
__global__ void build_part_ht_q23(const uint32_t* p_partkey, const uint32_t* p_brand1, int num_rows,
                                   uint32_t* ht_keys, uint32_t* ht_values, int ht_size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_rows) return;
...
__global__ void build_supplier_region_ht_q23(const uint32_t* s_suppkey, const uint32_t* s_region,
...
void runQ23Optimized(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    build_part_ht_q23<<<(P_LEN + block_size - 1) / block_size, block_size>>>(
    ...
    probeSingleFilterKernelQ23<<<grid_stage1, block_size>>>(d_suppkey, total_rows, ht_s_keys, ht_s_size, d_stage1_indices, d_num_stage1);
    cudaDeviceSynchronize();
    timer.stop();
    ...
    probePartWithExactBrandQ23<<<grid_stage2, block_size>>>(d_partkey, d_stage1_indices, h_num_stage1,
    ...
    aggregateQ23Kernel<<<grid_agg, block_size>>>(d_orderdate, d_revenue, d_stage2_brands, h_num_stage2,
    ...
    std::cout << "\n=== Q2.3 Results (OPTIMIZED) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.3 - OPTIMIZED ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ23Optimized(data, timing);

    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ23Optimized(data, t);
