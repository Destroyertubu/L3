/**
 * @file q43_optimized.cu
 * @brief SSB Q4.3 - OPTIMIZED: Decompress-first + Two-Level Fast Hash
 * Query: s_nation='UNITED STATES', p_category='MFGR#14', d_year 1997-1998
...
constexpr int AGG_SIZE = NUM_YEARS * NUM_CITIES;

__global__ void q43KernelOpt(
...
{
    uint32_t orderdate[ITEMS_PER_THREAD], suppkey[ITEMS_PER_THREAD], partkey[ITEMS_PER_THREAD];
    uint32_t revenue[ITEMS_PER_THREAD], supplycost[ITEMS_PER_THREAD];
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        ...
        // Part probe (category filter only)
        int p_slot;
        if (!twoLevelProbe(partkey[i], ht_p_keys, ht_p_size, p_slot)) {
            selection_flags[i] = 0; continue;
        }

        // Supplier probe (with city value)
        uint32_t s_city = 0;
        ...
        int agg_idx = year_idx * NUM_CITIES + s_city;
...
__global__ void build_part_category_ht(const uint32_t* pk, const uint32_t* pc, int n,
    uint32_t fc, uint32_t* k, int s) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n || pc[i] != fc) return;
    ...
    for (int p = 0; p < s; p++) { int sl = (slot + p) % s; old = atomicCAS(&k[sl], (uint32_t)HT_EMPTY, pk[i]); if (old == (uint32_t)HT_EMPTY || old == pk[i]) return; }
}
__global__ void build_supp_nation_ht(const uint32_t* sk, const uint32_t* sn, const uint32_t* sc, int n,
    uint32_t fn, uint32_t* k, uint32_t* v, int s) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n || sn[i] != fn) return;
    twoLevelInsert(sk[i], sc[i], k, v, s);
}

void runQ43Optimized(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    int ht_p_size = P_LEN * 2; uint32_t *ht_p_keys;
    cudaMalloc(&ht_p_keys, ht_p_size * sizeof(uint32_t));
    cudaMemset(ht_p_keys, 0xFF, ht_p_size * sizeof(uint32_t));
    build_part_category_ht<<<(P_LEN+bs-1)/bs, bs>>>(data.d_p_partkey, data.d_p_category, P_LEN, P_CATEGORY_MFGR14, ht_p_keys, ht_p_size);
    ...
    build_supp_nation_ht<<<(S_LEN+bs-1)/bs, bs>>>(data.d_s_suppkey, data.d_s_nation, data.d_s_city, S_LEN, S_NATION_US, ht_s_keys, ht_s_values, ht_s_size);
    ...
    timer.start();
    q43KernelOpt<<<(LO_LEN+TILE_SIZE-1)/TILE_SIZE, BLOCK_THREADS>>>(
        d_lo_orderdate, d_lo_suppkey, d_lo_partkey, d_lo_revenue, d_lo_supplycost, LO_LEN,
        ht_d_keys, ht_d_values, ht_d_size, ht_s_keys, ht_s_values, ht_s_size, ht_p_keys, ht_p_size, d_agg_profit);
    cudaDeviceSynchronize();
    timer.stop();
    ...
    cudaFree(d_lo_orderdate); cudaFree(d_lo_suppkey); cudaFree(d_lo_partkey);
    cudaFree(d_lo_revenue); cudaFree(d_lo_supplycost);
    cudaFree(ht_d_keys); cudaFree(ht_d_values); cudaFree(ht_p_keys);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.3 - OPTIMIZED ===" << std::endl;
    SSBDataCompressedVertical data; data.loadAndCompress(dir);
    QueryTiming t; runQ43Optimized(data, t);
    std::cout << "\n=== Benchmark ===" << std::endl;
    for (int i = 0; i < 3; i++) { QueryTiming x; runQ43Optimized(data, x); std::cout << "Run " << i+1 << ": " << x.total_ms << " ms\n"; }
    data.free(); return 0;
}
