/**
 * @file q34_optimized.cu
 * @brief SSB Q3.4 - Optimized. c_city in ('UNITED KI1','UNITED KI5'), s_city in same, d_yearmonth='Dec1997'
 */
...
__global__ void probeSupplierQ34Opt(const uint32_t* suppkeys, int n, const uint32_t* ht_keys,
...
__global__ void probeCustomerQ34Opt(const uint32_t* custkeys, const int* in_idx, const uint32_t* in_scity,
...
__global__ void aggregateQ34Opt(const uint32_t* orderdate, const uint32_t* revenue,
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        ...
        if (od >= DATE_DEC1997_START && od <= DATE_DEC1997_END) {
            uint32_t yr = 0;
            if (probeSharedDateCache(od, &dc, ht_d_size, yr)) {
...
void runQ34Optimized(SSBDataCompressedVertical& data, QueryTiming& timing) {
    ...
    timer.start(); probeSupplierQ34Opt<<<(N+bs-1)/bs, bs>>>(d_sk, N, hsk, hsv, hs, s1i, s1c, n1); cudaDeviceSynchronize(); timer.stop();
    ...
    timer.start(); probeCustomerQ34Opt<<<(h1+bs-1)/bs, bs>>>(d_ck, s1i, s1c, h1, hck, hcv, hc, s2i, s2cc, s2sc, n2); cudaDeviceSynchronize(); timer.stop();
    ...
    timer.start(); aggregateQ34Opt<<<min((h2+bs-1)/bs, 256), bs>>>(d_od, d_rv, s2cc, s2sc, h2, hdk, hdv, hd, agg); cudaDeviceSynchronize(); timer.stop();
    ...
    std::cout << "\n=== Q3.4 (OPTIMIZED) === Groups: " << grp << ", Total: " << tot << std::endl;
    timing.print("Q3.4");
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.4 - OPTIMIZED ===" << std::endl;
    SSBDataCompressedVertical data; data.loadAndCompress(dir);
    QueryTiming t; runQ34Optimized(data, t);
    std::cout << "\n=== Benchmark ===" << std::endl;
    for (int i = 0; i < 3; i++) { QueryTiming x; runQ34Optimized(data, x); std::cout << "Run " << i+1 << ": " << x.total_ms << " ms\n"; }
    data.free(); return 0;
}
