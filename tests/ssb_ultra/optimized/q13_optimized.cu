/**
 * @file q13_optimized.cu
 * @brief SSB Q1.3 Implementation - OPTIMIZED with multi-stream parallel decompression
 *
 * Query: SUM(lo_extendedprice * lo_discount) WHERE date in week 6 of 1994, discount 5-7, quantity 26-35
 *
 * Note: Q1.3 has very low selectivity (~0.2% for 1 week), but parallel decompress
...
__global__ void q13_kernel_optimized(
    ...
    unsigned long long* revenue)
{
    ...
    // Load orderdate and filter by week 6 of 1994
    BlockLoad(lo_orderdate + block_start, orderdate, block_items);
    BlockPredAndGTE(orderdate, Q13_DATE_MIN, selection_flags, block_items);
    BlockPredAndLTE(orderdate, Q13_DATE_MAX, selection_flags, block_items);
...
void runQ13Optimized(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    q13_kernel_optimized<<<grid_size, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q1.3 Results (OPTIMIZED) ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q1.3 - OPTIMIZED (Parallel Decompress) ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);

    QueryTiming timing;
    runQ13Optimized(data, timing);

    std::cout << "\n=== Benchmark Runs ===" << std::endl;
    for (int i = 0; i < 3; ++i) {
        QueryTiming t;
        runQ13Optimized(data, t);
