/**
 * @file q41.cu
 * @brief SSB Q4.1 Implementation - Decompress First Strategy
 *
 * Query:
...
void runQ41DecompressFirst(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    timer.start();
    q41_kernel<<<(LO_LEN+TILE_SIZE-1)/TILE_SIZE, BLOCK_THREADS>>>(d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_partkey, d_lo_revenue, d_lo_supplycost, LO_LEN,
    ...
    cudaFree(d_lo_orderdate); cudaFree(d_lo_custkey); cudaFree(d_lo_suppkey); cudaFree(d_lo_partkey); cudaFree(d_lo_revenue); cudaFree(d_lo_supplycost);
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.1 - Decompress First ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);
    QueryTiming timing;
    runQ41DecompressFirst(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ41DecompressFirst(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
