/**
 * @file q32.cu
 * @brief SSB Q3.2 Implementation - Decompress First Strategy
 *
 * Query:
...
constexpr uint32_t C_NATION_US = 24;  // UNITED STATES encoded
constexpr uint32_t S_NATION_US = 24;
constexpr int NUM_YEARS = 6;
constexpr int NUM_CITIES = 250;  // Max cities
constexpr int AGG_SIZE = NUM_YEARS * NUM_CITIES * NUM_CITIES;

__global__ void q32_kernel(
...
void runQ32DecompressFirst(SSBDataCompressedVertical& data, QueryTiming& timing) {
    CudaTimer timer;
    ...
    timer.start();
    q32_kernel<<<(LO_LEN+TILE_SIZE-1)/TILE_SIZE, BLOCK_THREADS>>>(d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_revenue, LO_LEN,
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q3.2 - Decompress First ===" << std::endl;
    SSBDataCompressedVertical data;
    data.loadAndCompress(data_dir);
    QueryTiming timing;
    runQ32DecompressFirst(data, timing);
    for (int i = 0; i < 3; ++i) { QueryTiming t; runQ32DecompressFirst(data, t); std::cout << "Run " << (i+1) << ": " << t.total_ms << " ms\n"; }
    data.free();
    return 0;
}
