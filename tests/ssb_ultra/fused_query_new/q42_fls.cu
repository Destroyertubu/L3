/**
 * @file q42_fls.cu
 * @brief SSB Q4.2 Implementation - Vertical-style FOR+BitPack
 *
 * Query:
...
// 2 years * 5 nations * 25 categories
// MFGR#1 categories: 11-15, MFGR#2 categories: 21-25 -> simplified to 10 categories
constexpr int NUM_YEARS = 2;
constexpr int NUM_NATIONS = 5;
constexpr int NUM_CATEGORIES = 10;  // category 11-15 -> 0-4, category 21-25 -> 5-9
constexpr int GROUP_SIZE = NUM_YEARS * NUM_NATIONS * NUM_CATEGORIES;
}
...
// Build part hash table for MFGR#1 OR MFGR#2, returns p_category
__global__ void build_part_pht_2_mfgr(
    ...
    int* ht, int ht_len, int keys_min)
{
    ...
    if (tid < num_rows) {
        ...
        if (m == mfgr1 || m == mfgr2) {
            int key = keys[tid];
            int cat = category[tid];
            // Map category: 11-15 -> 0-4, 21-25 -> 5-9
            int cat_idx = (cat >= 21) ? (cat - 21 + 5) : (cat - 11);
...
__global__ void build_date_pht_2_years(
    ...
    long long* __restrict__ group_result)
{
    ...
    // Step 2: lo_suppkey -> s_nation (AMERICA)
    {
    ...
    // Step 3: lo_partkey -> p_category (MFGR#1 OR MFGR#2)
    {
        ...
        BlockProbeAndPHT_2<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(
            items, p_cat, selection_flags, p_ht, p_ht_size, p_min_key, num_tile_items);
    }
    if (IsTerm<BLOCK_THREADS, ITEMS_PER_THREAD>(selection_flags)) return;
    ...
    // Aggregate: group_idx = year_idx * NUM_NATIONS * NUM_CATEGORIES + s_nation * NUM_CATEGORIES + p_cat
    #pragma unroll
    for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
        if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) && selection_flags[ITEM]) {
            ...
            int pc = p_cat[ITEM];
            if (y >= 0 && y < NUM_YEARS && sn >= 0 && sn < NUM_NATIONS && pc >= 0 && pc < NUM_CATEGORIES) {
                int idx = y * NUM_NATIONS * NUM_CATEGORIES + sn * NUM_CATEGORIES + pc;
...
void buildHashTables(SSBDataFLS& data,
    ...
    cudaMalloc(&d_s_ht, S_HT_SIZE * 2 * sizeof(int));   // PHT_2 -> nation
    cudaMalloc(&d_p_ht, P_HT_SIZE * 2 * sizeof(int));   // PHT_2 -> category
    ...
    // Supplier: region=AMERICA -> nation
    build_pht_2_kernel<int,int><<<(SSB_S_LEN+bs-1)/bs, bs>>>(
        data.d_s_suppkey, data.d_s_nation, data.d_s_region, S_REGION_AMERICA,
        SSB_S_LEN, d_s_ht, S_HT_SIZE, s_min_key);

    // Part: mfgr IN (1,2) -> category
    build_part_pht_2_mfgr<<<(SSB_P_LEN+bs-1)/bs, bs>>>(
        data.d_p_partkey, data.d_p_mfgr, data.d_p_category, P_MFGR_1, P_MFGR_2,
        SSB_P_LEN, d_p_ht, P_HT_SIZE, p_min_key);

    // Date: year IN (1997, 1998) -> year_idx
    build_date_pht_2_years<<<(SSB_D_LEN+bs-1)/bs, bs>>>(
...
void runQ42FLS(SSBDataFLS& data, FLSQueryTiming& timing) {
    ...
    std::cout << "Q4.2: " << num_blocks << " blocks" << std::endl;

    // Warmup + timed
    q42_fls_kernel<<<num_blocks, BLOCK_THREADS>>>(
    ...
    std::cout << "\n=== Q4.2 Average: " << timing.kernel_ms << " ms ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q4.2 - Vertical Style ===" << std::endl;
    SSBDataFLS data;
    data.loadAndEncode(data_dir);

    FLSQueryTiming timing;
    runQ42FLS(data, timing);
