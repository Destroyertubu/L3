/**
 * @file q22_fls.cu
 * @brief SSB Q2.2 Implementation - Vertical-style FOR+BitPack with Hash Joins
 *
 * Query:
...
namespace {
// Filter values
constexpr int P_BRAND1_MIN = 2221;  // 'MFGR#2221'
constexpr int P_BRAND1_MAX = 2228;  // 'MFGR#2228'
constexpr int S_REGION_ASIA = 2;    // 'ASIA' encoded as region 2
...
// Group result: 7 years * 8 brands (2221-2228)
constexpr int NUM_YEARS = 7;
constexpr int NUM_BRANDS = 8;
    ...
    unsigned long long* __restrict__ group_result)
{
    ...
    // Step 1: lo_suppkey - probe supplier hash table (ASIA)
    {
    ...
    // Step 2: lo_partkey - probe part hash table (brand1 2221-2228)
    {
    ...
    for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
        if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) && selection_flags[ITEM]) {
            ...
            int b = brand1[ITEM] - P_BRAND1_MIN;  // brands 2221-2228 -> 0-7
    ...
    int& s_min_key, int& p_min_key, int& d_min_key)
{
    ...
    // Supplier: WHERE s_region = 'ASIA'
    int s_num_blocks = (SSB_S_LEN + block_size - 1) / block_size;
    build_pht_1_kernel<int><<<s_num_blocks, block_size>>>(
        data.d_s_suppkey, data.d_s_region, S_REGION_ASIA,
        SSB_S_LEN, d_s_ht, S_HT_SIZE, s_min_key);

    // Part: WHERE p_brand1 BETWEEN 2221 AND 2228
    int p_num_blocks = (SSB_P_LEN + block_size - 1) / block_size;
    build_pht_2_range_kernel<int, int><<<p_num_blocks, block_size>>>(
        data.d_p_partkey, data.d_p_brand1, data.d_p_brand1, P_BRAND1_MIN, P_BRAND1_MAX,
        SSB_P_LEN, d_p_ht, P_HT_SIZE, p_min_key);

    // Date: return year (no filter)
    ...
    std::cout << "Hash tables built (Q2.2):" << std::endl;
    std::cout << "  Supplier: region=ASIA" << std::endl;
    std::cout << "  Part: brand1 in [2221, 2228]" << std::endl;
}
...
void runQ22FLS(SSBDataFLS& data, FLSQueryTiming& timing) {
    ...
    std::cout << "Q2.2 FLS Kernel Configuration:" << std::endl;
    std::cout << "  Blocks: " << num_blocks << ", Threads: " << BLOCK_THREADS << std::endl;

    // Warmup
    q22_fls_kernel<<<num_blocks, BLOCK_THREADS>>>(
        ...
        s_min_key, p_min_key, d_min_key, d_group_result);
    cudaDeviceSynchronize();
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== SSB Q2.2 - Vertical Style FOR+BitPack ===" << std::endl;

    SSBDataFLS data;
    data.loadAndEncode(data_dir);
    data.printStats();

    std::cout << "\n--- Running Q2.2 ---" << std::endl;
    FLSQueryTiming timing;
    runQ22FLS(data, timing);
