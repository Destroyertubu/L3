        ...
        for (int i = threadIdx.x; i < chunk_size; i += blockDim.x) {
            ...
            if constexpr (sizeof(T) == 8) {
                if (s_meta.delta_bits > 32) {
                    ...
                    if (s_meta.model_type == MODEL_DIRECT_COPY) {
                        // Apply sign extension only for signed types
                        if constexpr (std::is_signed<T>::value) {
                            int64_t signed_val = signExtend64(val64, s_meta.delta_bits);
                    ...
                    if (s_meta.model_type == MODEL_DIRECT_COPY) {
                        // Apply sign extension only for signed types
                        if constexpr (std::is_signed<T>::value) {
                            int32_t signed_val = signExtend(extracted, s_meta.delta_bits);
                            final_value = static_cast<T>(signed_val);
                        } else {
        ...
        if (s_model_type == MODEL_DIRECT_COPY) {
            ...
            if (s_delta_bits > 0) {
                ...
                if (s_delta_bits > 32 && sizeof(T) == 8) {
                    ...
                    // Apply sign extension only for signed types
                    if constexpr (std::is_signed<T>::value) {
                        int64_t signed_val = signExtend64(val64, s_delta_bits);
                        final_value = static_cast<T>(signed_val);
                    } else {
                        final_value = static_cast<T>(val64);
                    }
                } else {
                    // Extract raw bits without sign extension (to fix books_200M uint32 bug)
                    const int word_idx = static_cast<int>(bit_offset >> 5);
                    const int bit_in_word = static_cast<int>(bit_offset & 31);
                    const uint64_t combined = (static_cast<uint64_t>(compressed.delta_array[word_idx + 1]) << 32) |
                                               static_cast<uint64_t>(compressed.delta_array[word_idx]);
                    uint32_t extracted = (combined >> bit_in_word) & ((1ULL << s_delta_bits) - 1);

                    // Apply sign extension only for signed types (matching WarpOpt decoder)
                    if constexpr (std::is_signed<T>::value) {
                        int32_t signed_val = signExtend(extracted, s_delta_bits);
                        final_value = static_cast<T>(signed_val);
                    } else {
                        final_value = static_cast<T>(extracted);
                    }
