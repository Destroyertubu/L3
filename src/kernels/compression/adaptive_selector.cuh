    ...
    uint64_t& max_unsigned_delta)
{
    ...
    for (int i = start; i < end; i++) {
        ...
        int64_t residual = static_cast<int64_t>(val) - static_cast<int64_t>(std::llrint(predicted));
    ...
    // New residual = old_residual - min_residual >= 0
    // This is equivalent to: new_params[0] = old_params[0] + min_residual
...
// Legacy signed versions for backwards compatibility
template<typename T>
...
__device__ void fitLinear_d(int n, double sum_y, double sum_xy, double* params) {
    double dn = static_cast<double>(n);
    double sx = sumX_d(dn);
...
__global__ void computeStatsAndDecideFullPolynomial(
    ...
    // Declare shared memory for reductions - use separate arrays to avoid aliasing
    // Note: Use double for residuals to avoid overflow when T=uint64_t > INT64_MAX
    __shared__ double s_reduce_max[32];
    __shared__ double s_reduce_min[32];
    __shared__ double s_linear_max_pos_final, s_linear_max_neg_final;
    __shared__ double s_poly2_max_pos_final, s_poly2_max_neg_final;
    __shared__ double s_poly3_max_pos_final, s_poly3_max_neg_final;

    double linear_max_pos = -DBL_MAX, linear_max_neg = DBL_MAX;
    double poly2_max_pos = -DBL_MAX, poly2_max_neg = DBL_MAX;
    double poly3_max_pos = -DBL_MAX, poly3_max_neg = DBL_MAX;

    for (int i = start + threadIdx.x; i < end; i += blockDim.x) {
        T val = data[i];
        int local_idx = i - start;
        double x = static_cast<double>(local_idx);
        // Use double to avoid overflow for uint64 values > INT64_MAX
        double val_d = static_cast<double>(val);

        // LINEAR residual
        double pred_linear = s_linear_params[0] + s_linear_params[1] * x;
        double res_linear = val_d - pred_linear;
        linear_max_pos = fmax(linear_max_pos, res_linear);
        linear_max_neg = fmin(linear_max_neg, res_linear);

        // POLY2 residual (Horner's method)
        double pred_poly2 = s_poly2_params[0] + x * (s_poly2_params[1] + x * s_poly2_params[2]);
        double res_poly2 = val_d - pred_poly2;
        poly2_max_pos = fmax(poly2_max_pos, res_poly2);
        poly2_max_neg = fmin(poly2_max_neg, res_poly2);

        // POLY3 residual (Horner's method)
        double pred_poly3 = s_poly3_params[0] + x * (s_poly3_params[1] + x * (s_poly3_params[2] + x * s_poly3_params[3]));
        double res_poly3 = val_d - pred_poly3;
        poly3_max_pos = fmax(poly3_max_pos, res_poly3);
        poly3_max_neg = fmin(poly3_max_neg, res_poly3);
    }

    // Handle case where thread didn't process any data
    if (start + threadIdx.x >= end) {
        linear_max_pos = -DBL_MAX; linear_max_neg = DBL_MAX;
        poly2_max_pos = -DBL_MAX; poly2_max_neg = DBL_MAX;
        poly3_max_pos = -DBL_MAX; poly3_max_neg = DBL_MAX;
    }

    // Block reduce residual ranges with proper synchronization
    int lane = threadIdx.x & 31;
    int wid = threadIdx.x >> 5;
    ...
    if (threadIdx.x < 32) {
        s_reduce_max[threadIdx.x] = -DBL_MAX;
        s_reduce_min[threadIdx.x] = DBL_MAX;
    }
    __syncthreads();
    ...
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
        linear_max_pos = fmax(linear_max_pos, __shfl_down_sync(0xffffffff, linear_max_pos, offset));
        linear_max_neg = fmin(linear_max_neg, __shfl_down_sync(0xffffffff, linear_max_neg, offset));
    }
    if (lane == 0) {
    ...
    if (threadIdx.x < num_warps) {
        ...
        linear_max_pos = -DBL_MAX;
        linear_max_neg = DBL_MAX;
    }
    if (wid == 0) {
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
        poly2_max_pos = fmax(poly2_max_pos, __shfl_down_sync(0xffffffff, poly2_max_pos, offset));
        poly2_max_neg = fmin(poly2_max_neg, __shfl_down_sync(0xffffffff, poly2_max_neg, offset));
    }
    if (lane == 0) {
    ...
    if (threadIdx.x < num_warps) {
        ...
        poly2_max_pos = -DBL_MAX;
        poly2_max_neg = DBL_MAX;
    }
    if (wid == 0) {
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
        poly3_max_pos = fmax(poly3_max_pos, __shfl_down_sync(0xffffffff, poly3_max_pos, offset));
        poly3_max_neg = fmin(poly3_max_neg, __shfl_down_sync(0xffffffff, poly3_max_neg, offset));
    }
    if (lane == 0) {
    ...
    if (threadIdx.x < num_warps) {
        ...
        poly3_max_pos = -DBL_MAX;
        poly3_max_neg = DBL_MAX;
    }
    if (wid == 0) {
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    if (threadIdx.x == 0) {
        ...
        // Add safety margin for 64-bit types due to floating-point accumulation differences
        // between parallel sum reduction and serial prediction computation
        if (sizeof(T) == 8 && linear_bits > 0 && linear_bits < max_bits) {
            linear_bits = min(linear_bits + 4, max_bits);  // +4 bits safety margin
        }
        double cost_linear = LINEAR_METADATA_BYTES + dn * linear_bits / 8.0;

        // POLY2 cost
        int poly2_bits = computeResidualBits_d(poly2_max_pos, poly2_max_neg, max_bits);
        if (sizeof(T) == 8 && poly2_bits > 0 && poly2_bits < max_bits) {
            poly2_bits = min(poly2_bits + 4, max_bits);  // +4 bits safety margin
        }
        double cost_poly2 = POLY2_METADATA_BYTES + dn * poly2_bits / 8.0;

        // POLY3 cost
        int poly3_bits = computeResidualBits_d(poly3_max_pos, poly3_max_neg, max_bits);
        if (sizeof(T) == 8 && poly3_bits > 0 && poly3_bits < max_bits) {
            poly3_bits = min(poly3_bits + 4, max_bits);  // +4 bits safety margin
        }
...
__global__ void computeStatsAndDecide(
    ...
    if (threadIdx.x == 0) {
        ...
        if (cost_linear < cost_for * COST_THRESHOLD) {
...
__global__ void computeFixedModelKernel(
    ...
    if (actual_model == MODEL_FOR_BITPACK) {
        ...
        // Polynomial models: compute actual residuals
        // CRITICAL: Use double for residuals to handle uint64_t values > INT64_MAX
        double local_max_pos = -DBL_MAX;
        double local_max_neg = DBL_MAX;
        ...
        for (int i = start + threadIdx.x; i < end; i += blockDim.x) {
            ...
            switch (actual_model) {
                case MODEL_LINEAR:
                    predicted = s_params[0] + s_params[1] * x;
                    break;
                case MODEL_POLYNOMIAL2:
                    predicted = s_params[0] + x * (s_params[1] + x * s_params[2]);
                    break;
                case MODEL_POLYNOMIAL3:
                    predicted = s_params[0] + x * (s_params[1] + x * (s_params[2] + x * s_params[3]));
                    break;
                default:
                    predicted = s_params[0];
                    break;
            }

            double residual = val_d - predicted;
            local_max_pos = fmax(local_max_pos, residual);
            local_max_neg = fmin(local_max_neg, residual);
        }

        // Handle threads that didn't process any data
        if (start + threadIdx.x >= end) {
            local_max_pos = -DBL_MAX;
            local_max_neg = DBL_MAX;
        }

        // Reduce residual ranges with proper synchronization
        __shared__ double s_reduce_d[32];
        __shared__ double s_max_pos_final_d, s_max_neg_final_d;
        int lane = threadIdx.x & 31;
        int wid = threadIdx.x >> 5;
        int num_warps = (blockDim.x + 31) >> 5;
        ...
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1)
            local_max_pos = fmax(local_max_pos, __shfl_down_sync(0xffffffff, local_max_pos, offset));
        if (lane == 0) s_reduce_d[wid] = local_max_pos;
        __syncthreads();
        if (threadIdx.x < num_warps) local_max_pos = s_reduce_d[threadIdx.x];
        else local_max_pos = -DBL_MAX;
        if (wid == 0) {
            #pragma unroll
            for (int offset = 16; offset > 0; offset >>= 1)
                local_max_pos = fmax(local_max_pos, __shfl_down_sync(0xffffffff, local_max_pos, offset));
        }
        if (threadIdx.x == 0) s_max_pos_final_d = local_max_pos;
        __syncthreads();

        // Reduce max_neg (min)
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1)
            local_max_neg = fmin(local_max_neg, __shfl_down_sync(0xffffffff, local_max_neg, offset));
        if (lane == 0) s_reduce_d[wid] = local_max_neg;
        __syncthreads();
        if (threadIdx.x < num_warps) local_max_neg = s_reduce_d[threadIdx.x];
        else local_max_neg = DBL_MAX;
        if (wid == 0) {
            #pragma unroll
            for (int offset = 16; offset > 0; offset >>= 1)
                local_max_neg = fmin(local_max_neg, __shfl_down_sync(0xffffffff, local_max_neg, offset));
        }
        if (threadIdx.x == 0) s_max_neg_final_d = local_max_neg;
        __syncthreads();

        // Thread 0 writes final decision
        if (threadIdx.x == 0) {
            // Convert double residuals to int64 for bits computation
            int64_t max_pos_i64 = static_cast<int64_t>(ceil(s_max_pos_final_d));
            int64_t max_neg_i64 = static_cast<int64_t>(floor(s_max_neg_final_d));
            int bits = computeResidualBits_d(max_pos_i64, max_neg_i64, max_bits);
            // Add safety margin for 64-bit types due to floating-point precision issues
            if (sizeof(T) == 8 && bits > 0 && bits < max_bits) {
                bits = min(bits + 4, max_bits);  // +4 bits safety margin
            }
