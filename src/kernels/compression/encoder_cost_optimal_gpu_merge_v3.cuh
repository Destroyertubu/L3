...
#ifndef ENCODER_COST_OPTIMAL_GPU_MERGE_V3_CUH
#define ENCODER_COST_OPTIMAL_GPU_MERGE_V3_CUH
...
constexpr float GPU_MERGE_V3_MODEL_OVERHEAD_BYTES = 64.0f;
constexpr int GPU_MERGE_V3_BLOCK_SIZE = 256;
constexpr int GPU_MERGE_V3_MAX_BLOCKS = 512;  // Conservative limit for cooperative launch
...
constexpr uint64_t GPU_MERGE_V3_DOUBLE_PRECISION_MAX = 9007199254740992ULL;  // 2^53
...
template<typename T>
struct UnifiedPartitionBufferV3 {
    ...
    UnifiedPartitionBufferV3() : capacity(0), allocated(false),
...
template<typename T>
struct GPUMergeContextV3 {
    // Double buffers for ping-pong
    UnifiedPartitionBufferV3<T> buffer_A;
    UnifiedPartitionBufferV3<T> buffer_B;
    UnifiedPartitionBufferV3<T>* current;
    UnifiedPartitionBufferV3<T>* next;

    // Merge working arrays
    float* merge_benefits;
    int* merged_model_types;
    ...
    GPUMergeContextV3() : allocated(false), capacity(0),
        current(nullptr), next(nullptr),
        merge_benefits(nullptr), merged_model_types(nullptr), merged_delta_bits(nullptr),
        merged_theta0(nullptr), merged_theta1(nullptr),
        merged_theta2(nullptr), merged_theta3(nullptr),
        merged_costs(nullptr), merged_max_errors(nullptr),
...
struct GPUPartitionResultV3 {
    ...
    GPUPartitionResultV3() : d_starts(nullptr), d_ends(nullptr), d_model_types(nullptr),
...
template<typename T>
class GPUCostOptimalPartitionerV3 {
private:
    T* d_data;
...
template<typename T>
__global__ void mergeLoopCooperativeKernelV3(
...
// Compare CPU cost-optimal vs V3 performance
template<typename T>
...
#endif // ENCODER_COST_OPTIMAL_GPU_MERGE_V3_CUH
