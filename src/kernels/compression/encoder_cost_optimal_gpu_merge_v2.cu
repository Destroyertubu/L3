    ...
    for (int round = 0; round < max_rounds; round++) {
        ...
        if (pid < num_parts - 1) {
            ...
            if (n_c > max_partition_size) {
                ...
                // PASS 3: Compute max error using INT finite diff (matches encoder/decoder exactly)
    ...
    int data_size)
{
    ...
    // Compute max error in parallel using INT finite diff
    double linear_params[2] = {s_theta0, s_theta1};
    int64_t linear_y, linear_step;
    FiniteDiff::computeLinearINT<T>(linear_params, threadIdx.x, blockDim.x, linear_y, linear_step);
...
extern __global__ void gpuMergeCountPartitionsInSegmentsKernel(const int*, int, int, int, int, int*);
extern __global__ void gpuMergeWritePartitionsKernel(const int*, int, int, int, int, const int*, int*, int*);
