    ...
    int max_partition_size)
{
    ...
    // Single pass to compute max_error using INT finite diff (matches encoder/decoder exactly)
    int merged_start = start_a;
    int merged_end = end_b;

    // Initialize INT finite diff state
    double linear_params[2] = {s_theta0, s_theta1};
    int64_t linear_y, linear_step;
    FiniteDiff::computeLinearINT<T>(linear_params, threadIdx.x, blockDim.x, linear_y, linear_step);
    ...
    for (int i = merged_start + threadIdx.x; i < merged_end; i += blockDim.x) {
        T val = data[i];
        T pred_val = static_cast<T>(linear_y);
        long long delta;
        if (val >= pred_val) {
        ...
        linear_y += linear_step;  // INT accumulation
    }
    ...
    int num_blocks)
{
    ...
    // Compute max error using INT finite diff (matches encoder/decoder exactly)
    double linear_params[2] = {s_theta0, s_theta1};
    int64_t linear_y, linear_step;
    FiniteDiff::computeLinearINT<T>(linear_params, threadIdx.x, blockDim.x, linear_y, linear_step);
    ...
    int num_partitions)
{
    ...
    // CRITICAL: Compute max_error using INT finite diff with ANCHORED prediction
    // theta0 = data[start] (anchored), matches encoder exactly
    double anchored_theta0 = static_cast<double>(data[start]);
    double anchored_params[2] = {anchored_theta0, s_theta1};
    int64_t linear_y, linear_step;
    FiniteDiff::computeLinearINT<T>(anchored_params, threadIdx.x, blockDim.x, linear_y, linear_step);
    ...
    float cost_threshold)   // Require this much improvement (e.g., 0.95 = 5% better)
{
    ...
    // Compute max errors for all models using INT finite diff (matches encoder/decoder exactly)
    // Initialize INT finite diff state for each model
    int64_t linear_y, linear_step;
    FiniteDiff::computeLinearINT<T>(s_linear_params, threadIdx.x, blockDim.x, linear_y, linear_step);

    int64_t poly2_y, poly2_d1, poly2_d2;
    FiniteDiff::computePoly2INT<T>(s_poly2_params, threadIdx.x, blockDim.x, poly2_y, poly2_d1, poly2_d2);

    int64_t poly3_y, poly3_d1, poly3_d2, poly3_d3;
    FiniteDiff::computePoly3INT<T>(s_poly3_params, threadIdx.x, blockDim.x, poly3_y, poly3_d1, poly3_d2, poly3_d3);
    ...
    for (int i = start + threadIdx.x; i < end; i += blockDim.x) {
        ...
        // LINEAR prediction
        T pv_linear = static_cast<T>(linear_y);
        ...
        // POLY2 prediction
        T pv_poly2 = static_cast<T>(poly2_y);
        ...
        // POLY3 prediction
        T pv_poly3 = static_cast<T>(poly3_y);
        ...
        // INT accumulation
        linear_y += linear_step;
        poly2_y += poly2_d1; poly2_d1 += poly2_d2;
        poly3_y += poly3_d1; poly3_d1 += poly3_d2; poly3_d2 += poly3_d3;
    }
