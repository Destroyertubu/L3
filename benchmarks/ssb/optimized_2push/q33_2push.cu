// SSB Query 3.3 with L32 Compression - RANDOM ACCESS VERSION (_2push version)
// Query: SELECT c_city, s_city, d_year, SUM(lo_revenue) AS revenue
//        FROM customer, lineorder, supplier, date
//        WHERE lo_custkey = c_custkey
//              AND lo_suppkey = s_suppkey
//              AND lo_orderdate = d_datekey
//              AND c_city IN ('UNITED KI1', 'UNITED KI5') (encoded as 101, 105)
//              AND s_city IN ('UNITED KI1', 'UNITED KI5') (encoded as 101, 105)
...
template<typename T>
__global__ void ssb_q33_stage1_scan(
    ...
    for (int local_idx = threadIdx.x; local_idx < s_meta_custkey.partition_len; local_idx += blockDim.x) {
        ...
        if (c_city != 101 && c_city != 105) continue;
        ...
        if (s_city != 101 && s_city != 105) continue;

        // Add to local candidate buffer
        int pos = atomicAdd(&s_local_count, 1);
        if (pos < 256) {
            s_local_candidates[pos] = {global_idx, c_city, s_city};
        } else {
            ...
            if (threadIdx.x == 0) {
                ...
                s_local_candidates[0] = {global_idx, c_city, s_city};
                s_local_count = 1;
            }
            __syncthreads();
        }
...
template<typename T>
__global__ void ssb_q33_stage2_random_access(
...
int main(int argc, char** argv) {
    cout << "========================================================================" << endl;
    cout << "  SSB Query 3.3 with L32 Compression - RANDOM ACCESS" << endl;
    cout << "========================================================================" << endl;
    cout << endl;
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        ssb_q33_stage1_scan<uint32_t><<<c_custkey_c->num_partitions, 256>>>(
        ...
        if (h_candidate_count > 0) {
            ...
            ssb_q33_stage2_random_access<uint32_t><<<blocks, 256>>>(
        ...
        cout << "{\"query\":33,\"time_query\":" << query_time
