// SSB Query 4.2 with L32 Compression - RANDOM ACCESS VERSION (_2push version)
// Stage 1: Scan orderdate, partkey, custkey, suppkey (4 cols)
// Stage 2: Random access revenue, supplycost (2 cols)
...
struct GroupKey {
    ...
    bool operator<(const GroupKey& other) const {
        ...
        if (s_nation != other.s_nation) return s_nation < other.s_nation;
        return p_category < other.p_category;
    }
};

template<typename T>
__global__ void ssb_q42_stage1_scan(
    ...
    __shared__ PartitionMetaOpt s_meta[4];
    ...
    if (threadIdx.x == 0) {
        s_local_count = 0;
        const CompressedDataL3<T>* data_arr[4] = {c_orderdate, c_partkey, c_custkey, c_suppkey};
        for (int i = 0; i < 4; i++) {
            s_meta[i].start_idx = data_arr[i]->d_start_indices[partition_idx];
            s_meta[i].delta_bits = data_arr[i]->d_delta_bits[partition_idx];
            s_meta[i].bit_offset_base = data_arr[i]->d_delta_array_bit_offsets[partition_idx];
            s_meta[i].theta0 = data_arr[i]->d_model_params[partition_idx * 4];
            s_meta[i].theta1 = data_arr[i]->d_model_params[partition_idx * 4 + 1];
            s_meta[i].partition_len = data_arr[i]->d_end_indices[partition_idx] - s_meta[i].start_idx;
        }
    }
    ...
    for (int local_idx = threadIdx.x; local_idx < s_meta[0].partition_len; local_idx += blockDim.x) {
        ...
        if (c_orderdate->d_plain_deltas != nullptr) {
            ...
            delta = extractDelta_Optimized<T>(c_orderdate->delta_array, bit_offset, s_meta[0].delta_bits);
        }
        predicted = fma(s_meta[0].theta1, static_cast<double>(local_idx), s_meta[0].theta0);
        ...
        if (c_partkey->d_plain_deltas != nullptr) {
            ...
            delta = extractDelta_Optimized<T>(c_partkey->delta_array, bit_offset, s_meta[1].delta_bits);
        }
        ...
        uint32_t p_category = d_part_ht[partkey].p_category;
        ...
        if (c_custkey->d_plain_deltas != nullptr) {
            ...
            delta = extractDelta_Optimized<T>(c_custkey->delta_array, bit_offset, s_meta[2].delta_bits);
        }
        ...
        } else if (s_meta[3].delta_bits > 0) {
            int64_t bit_offset = s_meta[3].bit_offset_base + (int64_t)local_idx * s_meta[3].delta_bits;
            delta = extractDelta_Optimized<T>(c_suppkey->delta_array, bit_offset, s_meta[3].delta_bits);
        }
        predicted = fma(s_meta[3].theta1, static_cast<double>(local_idx), s_meta[3].theta0);
        ...
        uint32_t s_nation = d_supplier_ht[suppkey].s_nation;

        int pos = atomicAdd(&s_local_count, 1);
        if (pos < 256) {
            s_local_candidates[pos] = {global_idx, d_year, s_nation, p_category};
        } else {
            __syncthreads();
            if (threadIdx.x == 0) {
                ...
                s_local_candidates[0] = {global_idx, d_year, s_nation, p_category};
                s_local_count = 1;
            }
            __syncthreads();
        }
...
template<typename T>
__global__ void ssb_q42_stage2_random_access(
    ...
    uint64_t combined_key = ((uint64_t)cand.d_year << 32) | ((uint64_t)cand.s_nation << 16) | cand.p_category;
    ...
    for (int probe = 0; probe < 64; probe++) {
        ...
        if (existing_key == 0) {
            ...
            if (old == 0 || old == combined_key) {
                d_group_years[gidx] = cand.d_year;
                d_group_snations[gidx] = cand.s_nation;
                d_group_categories[gidx] = cand.p_category;
                group_idx = gidx;
                break;
            }
...
int main(int argc, char** argv) {
    cout << "========================================================================" << endl;
    cout << "  SSB Query 4.2 with L32 Compression - RANDOM ACCESS" << endl;
    cout << "========================================================================" << endl;
    cout << endl;
    ...
    CompressedDataL3<uint32_t>* c_orderdate = compressData(lo_orderdate, 2048);
    CompressedDataL3<uint32_t>* c_custkey_c = compressData(lo_custkey, 2048);
    CompressedDataL3<uint32_t>* c_suppkey_c = compressData(lo_suppkey, 2048);
    CompressedDataL3<uint32_t>* c_partkey_c = compressData(lo_partkey, 2048);
    CompressedDataL3<uint32_t>* c_revenue = compressData(lo_revenue, 2048);
    CompressedDataL3<uint32_t>* c_supplycost_c = compressData(lo_supplycost, 2048);
    ...
    CUDA_CHECK(cudaMalloc(&d_group_categories, MAX_GROUPS * sizeof(uint32_t)));
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        CUDA_CHECK(cudaMemset(d_group_categories, 0, MAX_GROUPS * sizeof(uint32_t)));
        CUDA_CHECK(cudaMemset(d_group_keys, 0, MAX_GROUPS * sizeof(uint64_t)));

        auto query_start = chrono::high_resolution_clock::now();

        ssb_q42_stage1_scan<uint32_t><<<c_orderdate->num_partitions, 256>>>(
            c_orderdate->d_self, c_partkey_c->d_self, c_custkey_c->d_self, c_suppkey_c->d_self,
            d_date_ht, d_part_ht, d_customer_ht, d_supplier_ht, LO_LEN,
        ...
        if (h_candidate_count > 0) {
            ...
            ssb_q42_stage2_random_access<uint32_t><<<blocks, 256>>>(
                c_revenue->d_self, c_supplycost_c->d_self,
                d_candidates, h_candidate_count,
                d_group_results, d_group_years, d_group_snations, d_group_categories, d_group_keys, MAX_GROUPS
            );
        }
        ...
        vector<uint32_t> h_group_categories(MAX_GROUPS);
        ...
        CUDA_CHECK(cudaMemcpy(h_group_categories.data(), d_group_categories, MAX_GROUPS * sizeof(uint32_t), cudaMemcpyDeviceToHost));

        map<GroupKey, unsigned long long> results_map;
        for (int i = 0; i < MAX_GROUPS; i++) {
            if (h_group_years[i] != 0) {
                GroupKey key{h_group_years[i], h_group_snations[i], h_group_categories[i]};
                results_map[key] += h_group_results[i];
            }
        }

        double selectivity = (double)h_candidate_count / LO_LEN * 100.0;
        cout << "{\"query\":42,\"time_query\":" << query_time
        ...
        if (t == 0) {
            cout << "Sample results (d_year, s_nation, p_category, profit):" << endl;
            int count = 0;
            for (const auto& [key, profit] : results_map) {
                cout << "  " << key.d_year << ", " << key.s_nation << ", " << key.p_category << ": " << profit << endl;
                if (++count >= 10) break;
            }
        }
    }
    ...
    CUDA_CHECK(cudaFree(d_group_categories));
