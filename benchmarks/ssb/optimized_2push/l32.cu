...
// Main L3GPU class - UPDATED FOR SoA
template<typename T>
class L3GPU {
private:
...
void run_compression_test(const std::vector<T>& data_to_test, 
    ...
    compression_time_ms = benchmark([&]() {
        if (compressed_data) {
            l3.cleanup(compressed_data);
            compressed_data = nullptr;
        }
        // MODIFICATION 16: Pass new parameters to compress function
        compressed_data = l3.compress(data_to_test, true, &compressed_size_bytes, true, dataset_name, variance_block_multiplier, num_thresholds);
    }, 5); 
    ...
    double decompression_time_ms = benchmark([&]() {
        l3.decompressFullFile_OnTheFly_Optimized_V2(compressed_data, decompressed_data);
    }, 100);
    ...
    double serialization_time_ms = benchmark([&]() {
        ...
        serialized_blob = l3.serializeGPU(compressed_data);
    }, 20);
    ...
    double deserialization_time_ms = benchmark([&]() {
        if(deserialized_data) l3.cleanup(deserialized_data);
        deserialized_data = l3.deserializeGPU(serialized_blob, false);
    }, 100);
    ...
    l3.decompressFullFile_OnTheFly_Optimized_V2(deserialized_data, decompressed_from_deserialized);
    ...
    double deserialization_preunpack_time_ms = benchmark([&]() {
        if(deserialized_preunpacked_data) l3.cleanup(deserialized_preunpacked_data);
        deserialized_preunpacked_data = l3.deserializeGPU(serialized_blob, true);
    }, 100);
    ...
    double decompression_preunpacked_time_ms = benchmark([&]() {
        l3.decompressFullFile_OnTheFly_Optimized_V2(deserialized_preunpacked_data, decompressed_preunpacked);
    }, 100);
    ...
    l3.cleanup(compressed_data);
    l3.cleanup(deserialized_data);
    l3.cleanup(deserialized_preunpacked_data);
    if (serialized_blob) {
        delete serialized_blob;
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "L3 GPU Compression Test - Integer Focus" << std::endl;
