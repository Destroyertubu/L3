// SSB Query 1.2 with Optimized L3 Compression
// SELECT SUM(lo_extendedprice * lo_discount) AS revenue
       // FROM lineorder
       // WHERE lo_orderdate >= 19940000 AND lo_orderdate < 19950000
    ...
    unsigned long long* revenue)
{
    ...
    // Apply filters
    if (orderdate >= 19940000 && orderdate < 19950000 &&
        discount >= 4 && discount <= 6 &&
        quantity >= 26 && quantity <= 35) {
...
int main(int argc, char** argv) {
    cout << "========================================================================" << endl;
    cout << "  SSB Query 1.2 with Optimized L3 Compression" << endl;
    cout << "========================================================================" << endl;
    cout << endl;

    cout << "Query: SELECT SUM(lo_extendedprice * lo_discount) AS revenue" << endl;
    cout << "       FROM lineorder" << endl;
    cout << "       WHERE lo_orderdate >= 19940000 AND lo_orderdate < 19950000" << endl;
    cout << "             AND lo_discount >= 4 AND lo_discount <= 6" << endl;
    cout << "             AND lo_quantity >= 26 AND lo_quantity <= 35" << endl;
    cout << endl;
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        decompressData(c_orderdate, d_orderdate, LO_LEN, &decomp_stats_tmp);
        decompressData(c_discount, d_discount, LO_LEN, &decomp_stats_tmp);
        decompressData(c_quantity, d_quantity, LO_LEN, &decomp_stats_tmp);
        decompressData(c_extendedprice, d_extendedprice, LO_LEN, &decomp_stats_tmp);
        CUDA_CHECK(cudaDeviceSynchronize());

        // Run query kernel
        int block_size = 256;
        int grid_size = (LO_LEN + block_size - 1) / block_size;
        ssb_q12_kernel<<<grid_size, block_size>>>(
        ...
        cout << "{\"query\":12,\"time_ms\":" << query_time
