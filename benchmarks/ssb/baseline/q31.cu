// SSB Query 3.1 with L32 Compression - RANDOM ACCESS VERSION
...
template<typename T>
__global__ void ssb_q31_stage1_scan(
    ...
    for (int local_idx = threadIdx.x; local_idx < s_meta_custkey.partition_len; local_idx += blockDim.x) {
        ...
        if (cust.c_region != 2) continue; // c_region = 'ASIA'
        ...
        if (supp.s_region != 2) continue; // s_region = 'ASIA'

        // Add to local candidate buffer
        int pos = atomicAdd(&s_local_count, 1);
        if (pos < 256) {
            s_local_candidates[pos] = {global_idx, cust.c_nation, supp.s_nation};
        } else {
            ...
            if (threadIdx.x == 0) {
                ...
                s_local_candidates[0] = {global_idx, cust.c_nation, supp.s_nation};
                s_local_count = 1;
            }
            __syncthreads();
        }
...
template<typename T>
__global__ void ssb_q31_stage2_random_access(
    ...
    // Hash-based GROUP BY
    uint64_t combined_key = ((uint64_t)cand.c_nation << 32) | ((uint64_t)cand.s_nation << 16) | d_year;
    ...
    for (int probe = 0; probe < 64; probe++) {
        ...
        if (d_group_cnations[gidx] == cand.c_nation &&
        ...
        if (d_group_cnations[gidx] == 0) {
            unsigned int old = atomicCAS(&d_group_cnations[gidx], 0, cand.c_nation);
            if (old == 0) {
                d_group_snations[gidx] = cand.s_nation;
                d_group_years[gidx] = d_year;
                group_idx = gidx;
                break;
            } else if (old == cand.c_nation &&
                       d_group_snations[gidx] == cand.s_nation &&
                       d_group_years[gidx] == d_year) {
                group_idx = gidx;
                break;
...
int main(int argc, char** argv) {
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        ssb_q31_stage1_scan<uint32_t><<<c_custkey_c->num_partitions, 256>>>(
        ...
        if (h_candidate_count > 0) {
            ...
            ssb_q31_stage2_random_access<uint32_t><<<blocks, 256>>>(
                c_orderdate->d_self, c_revenue->d_self,
                d_date_ht, d_candidates, h_candidate_count,
                d_group_results, d_group_cnations, d_group_snations, d_group_years, MAX_GROUPS
            );
        }
        ...
        cout << "{\"query\":31,\"time_query\":" << query_time
