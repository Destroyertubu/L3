...
typedef struct _TResultSetBufferDescriptor__isset {
  _TResultSetBufferDescriptor__isset() : layout(false), keyless(false), entry_count(false), idx_target_as_key(false), min_val(false), max_val(false), bucket(false), group_col_widths(false), key_bytewidth(false), col_slot_context(false), target_groupby_indices(false), count_distinct_descriptors(false), force_4byte_float(false), gpu_shared_mem_used(false) {}
  bool layout :1;
  bool keyless :1;
  bool entry_count :1;
...
class TResultSetBufferDescriptor : public virtual ::apache::thrift::TBase {
 public:
  ...
  void __set_gpu_shared_mem_used(const bool val);
  ...
  bool operator == (const TResultSetBufferDescriptor & rhs) const
  {
    ...
    if (!(force_4byte_float == rhs.force_4byte_float))
      return false;
    if (!(gpu_shared_mem_used == rhs.gpu_shared_mem_used))
      return false;
    return true;
  }
  bool operator != (const TResultSetBufferDescriptor &rhs) const {
