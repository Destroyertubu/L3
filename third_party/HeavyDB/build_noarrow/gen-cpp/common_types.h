/**
 * Autogenerated by Thrift Compiler (0.19.0)
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */
#ifndef common_TYPES_H
#define common_TYPES_H

#include <iosfwd>

#include <thrift/Thrift.h>
#include <thrift/TApplicationException.h>
#include <thrift/TBase.h>
#include <thrift/protocol/TProtocol.h>
#include <thrift/transport/TTransport.h>

#include <functional>
#include <memory>




struct TDeviceType {
  enum type {
    CPU = 0,
    GPU = 1
  };
};

extern const std::map<int, const char*> _TDeviceType_VALUES_TO_NAMES;

std::ostream& operator<<(std::ostream& out, const TDeviceType::type& val);

std::string to_string(const TDeviceType::type& val);

struct TDatumType {
  enum type {
    SMALLINT = 0,
    INT = 1,
    BIGINT = 2,
    FLOAT = 3,
    DECIMAL = 4,
    DOUBLE = 5,
    STR = 6,
    TIME = 7,
    TIMESTAMP = 8,
    DATE = 9,
    BOOL = 10,
    INTERVAL_DAY_TIME = 11,
    INTERVAL_YEAR_MONTH = 12,
    POINT = 13,
    LINESTRING = 14,
    POLYGON = 15,
    MULTIPOLYGON = 16,
    TINYINT = 17,
    GEOMETRY = 18,
    GEOGRAPHY = 19,
    MULTILINESTRING = 20,
    MULTIPOINT = 21
  };
};

extern const std::map<int, const char*> _TDatumType_VALUES_TO_NAMES;

std::ostream& operator<<(std::ostream& out, const TDatumType::type& val);

std::string to_string(const TDatumType::type& val);

struct TEncodingType {
  enum type {
    NONE = 0,
    FIXED = 1,
    RL = 2,
    DIFF = 3,
    DICT = 4,
    SPARSE = 5,
    GEOINT = 6,
    DATE_IN_DAYS = 7,
    ARRAY = 8,
    ARRAY_DICT = 9
  };
};

extern const std::map<int, const char*> _TEncodingType_VALUES_TO_NAMES;

std::ostream& operator<<(std::ostream& out, const TEncodingType::type& val);

std::string to_string(const TEncodingType::type& val);

class TStringDictKey;

class TTypeInfo;

typedef struct _TStringDictKey__isset {
  _TStringDictKey__isset() : db_id(false), dict_id(false) {}
  bool db_id :1;
  bool dict_id :1;
} _TStringDictKey__isset;

class TStringDictKey : public virtual ::apache::thrift::TBase {
 public:

  TStringDictKey(const TStringDictKey&) noexcept;
  TStringDictKey& operator=(const TStringDictKey&) noexcept;
  TStringDictKey() noexcept
                 : db_id(0),
                   dict_id(0) {
  }

  virtual ~TStringDictKey() noexcept;
  int32_t db_id;
  int32_t dict_id;

  _TStringDictKey__isset __isset;

  void __set_db_id(const int32_t val);

  void __set_dict_id(const int32_t val);

  bool operator == (const TStringDictKey & rhs) const
  {
    if (!(db_id == rhs.db_id))
      return false;
    if (!(dict_id == rhs.dict_id))
      return false;
    return true;
  }
  bool operator != (const TStringDictKey &rhs) const {
    return !(*this == rhs);
  }

  bool operator < (const TStringDictKey & ) const;

  uint32_t read(::apache::thrift::protocol::TProtocol* iprot) override;
  uint32_t write(::apache::thrift::protocol::TProtocol* oprot) const override;

  virtual void printTo(std::ostream& out) const;
};

void swap(TStringDictKey &a, TStringDictKey &b);

std::ostream& operator<<(std::ostream& out, const TStringDictKey& obj);

typedef struct _TTypeInfo__isset {
  _TTypeInfo__isset() : type(false), encoding(false), nullable(false), is_array(false), precision(false), scale(false), comp_param(false), size(true), dict_key(false) {}
  bool type :1;
  bool encoding :1;
  bool nullable :1;
  bool is_array :1;
  bool precision :1;
  bool scale :1;
  bool comp_param :1;
  bool size :1;
  bool dict_key :1;
} _TTypeInfo__isset;

class TTypeInfo : public virtual ::apache::thrift::TBase {
 public:

  TTypeInfo(const TTypeInfo&) noexcept;
  TTypeInfo& operator=(const TTypeInfo&) noexcept;
  TTypeInfo() noexcept
            : type(static_cast<TDatumType::type>(0)),
              encoding(static_cast<TEncodingType::type>(0)),
              nullable(0),
              is_array(0),
              precision(0),
              scale(0),
              comp_param(0),
              size(-1) {
  }

  virtual ~TTypeInfo() noexcept;
  /**
   * 
   * @see TDatumType
   */
  TDatumType::type type;
  /**
   * 
   * @see TEncodingType
   */
  TEncodingType::type encoding;
  bool nullable;
  bool is_array;
  int32_t precision;
  int32_t scale;
  int32_t comp_param;
  int32_t size;
  TStringDictKey dict_key;

  _TTypeInfo__isset __isset;

  void __set_type(const TDatumType::type val);

  void __set_encoding(const TEncodingType::type val);

  void __set_nullable(const bool val);

  void __set_is_array(const bool val);

  void __set_precision(const int32_t val);

  void __set_scale(const int32_t val);

  void __set_comp_param(const int32_t val);

  void __set_size(const int32_t val);

  void __set_dict_key(const TStringDictKey& val);

  bool operator == (const TTypeInfo & rhs) const
  {
    if (!(type == rhs.type))
      return false;
    if (!(encoding == rhs.encoding))
      return false;
    if (!(nullable == rhs.nullable))
      return false;
    if (!(is_array == rhs.is_array))
      return false;
    if (!(precision == rhs.precision))
      return false;
    if (!(scale == rhs.scale))
      return false;
    if (!(comp_param == rhs.comp_param))
      return false;
    if (__isset.size != rhs.__isset.size)
      return false;
    else if (__isset.size && !(size == rhs.size))
      return false;
    if (__isset.dict_key != rhs.__isset.dict_key)
      return false;
    else if (__isset.dict_key && !(dict_key == rhs.dict_key))
      return false;
    return true;
  }
  bool operator != (const TTypeInfo &rhs) const {
    return !(*this == rhs);
  }

  bool operator < (const TTypeInfo & ) const;

  uint32_t read(::apache::thrift::protocol::TProtocol* iprot) override;
  uint32_t write(::apache::thrift::protocol::TProtocol* oprot) const override;

  virtual void printTo(std::ostream& out) const;
};

void swap(TTypeInfo &a, TTypeInfo &b);

std::ostream& operator<<(std::ostream& out, const TTypeInfo& obj);



#endif
