...
where lo_custkey = c_custkey
  ...
  and c_nation = 'UNITED STATES'  -- nation = 24
  and s_nation = 'UNITED STATES'  -- nation = 24
  and (d_year = 1997 or d_year = 1998)
  and p_category = 'MFGR#14'  -- category = 13
...
__global__ void probe(
  ...
  // 1. Probe supplier hash table - returns s_city
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    ...
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items)) {
      ...
      if (slot != 0) {
        s_city[ITEM] = ht_s[(hash << 1) + 1];  // get s_city
      } else {
  ...
  // 2. Probe customer hash table - returns c_city
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items)) {
      ...
      if (selection_flags[ITEM]) {
        ...
        if (slot != 0) {
          c_city[ITEM] = ht_c[(hash << 1) + 1];  // get c_city
        } else {
          selection_flags[ITEM] = 0;
        }
      }
  ...
  // 3. Probe part hash table - returns p_brand1
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items)) {
      ...
      if (selection_flags[ITEM]) {
        ...
        if (slot != 0) {
          p_brand[ITEM] = ht_p[(hash << 1) + 1];  // get p_brand1
        } else {
          selection_flags[ITEM] = 0;
        }
      }
  ...
  // Q43 aggregation: by (year, s_city, p_brand)
  // Use hash to reduce space - 2 years × 250 cities × 1000 brands = 500000
  #pragma unroll
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items)) {
      if (selection_flags[ITEM]) {
        int hash = ((year[ITEM] - 1992) * NUM_CITIES * NUM_BRANDS + s_city[ITEM] * NUM_BRANDS + p_brand[ITEM]) % res_size;
        res[hash * 6] = year[ITEM];
        res[hash * 6 + 1] = s_city[ITEM];
        res[hash * 6 + 2] = p_brand[ITEM];
...
// Q43: s_nation = 24 (US), return s_city
__global__
...
void build_hashtable_p(int *filter_col, int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots) {
  ...
  if (offset < num_tuples) {
    int val = filter_col[offset];
    if (val == 13) {  // category = 13
      int key = dim_key[offset];
      int hash = HASH(key, num_slots);
      int init = 0;

      int old = atomicCAS(&hash_table[hash << 1], init, key);
      hash_table[(hash << 1) + 1] = dim_val[offset];  // p_brand1
    }
  }
}

// Q43: c_nation = 24 (US), return c_city
__global__
void build_hashtable_c(int* filter_col, int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots) {
  ...
  if (offset < num_tuples) {
    ...
    if (f == 24) {  // UNITED STATES = 24
      int key = dim_key[offset];
      int hash = HASH(key, num_slots);
      int init = 0;

      int old = atomicCAS(&hash_table[hash << 1], init, key);
      hash_table[(hash << 1) + 1] = dim_val[offset];  // c_city
    }
  }
}

// Q43: d_year IN {1997, 1998}
__global__
...
float runQuery(encoded_column lo_orderdate_val, encoded_column lo_orderdate_rl, encoded_column lo_custkey_val, encoded_column lo_custkey_rl, encoded_column lo_partkey,
  ...
  // Q43: s_nation=24, return s_city
  build_hashtable_s<<<(s_len + 127)/128, 128>>>(s_nation, s_suppkey, s_city, s_len, ht_s, s_len);

  // Q43: c_nation=24, return c_city
  build_hashtable_c<<<(c_len + 127)/128, 128>>>(c_nation, c_custkey, c_city, c_len, ht_c, c_len);

  // Q43: p_category=13, return p_brand1
  ...
  int *res;
  // Q43 result: 2 years × 250 cities × 1000 brands = 500000 (use hash)
  int res_size = 2 * NUM_CITIES * NUM_BRANDS;
  int ht_entries = 6; // year, s_city, p_brand, padding, long long profit
  ...
  probe<num_threads, items_per_thread><<<(lo_len + tile_size - 1)/tile_size, 128>>>(
      ...
      lo_len,
      ht_p, p_len, ht_s, s_len, ht_c, c_len, ht_d, d_val_len, res, res_size);
...
int main(int argc, char** argv)
{
  ...
  int *d_s_city = loadColumnToGPU<int>(h_s_city, S_LEN, g_allocator);
  ...
  int *d_c_city = loadColumnToGPU<int>(h_c_city, C_LEN, g_allocator);
