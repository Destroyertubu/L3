...
where lo_orderdate = d_datekey
  ...
  and p_brand1 between 'MFGR#2221' and 'MFGR#2228'  -- brand1 IN [260, 267]
  and s_region = 'ASIA'  -- region = 2
...
// Q22: p_brand1 IN [260, 267] ('MFGR#2221' to 'MFGR#2228')
__global__
void build_hashtable_p(int *filter_col, int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots) {
  int offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset < num_tuples) {
    int brand = filter_col[offset];
    if (brand >= 260 && brand <= 267) {  // p_brand1 IN [260, 267]
      int key = dim_key[offset];
...
float runQuery(encoded_column lo_orderdate_val, encoded_column lo_orderdate_rl,
  ...
  // Q22: filter on p_brand1, return p_brand1 as value
  build_hashtable_p<<<(p_len + 127)/128, 128>>>(p_brand1, p_partkey, p_brand1, p_len, ht_p, p_len);
  /*CHECK_ERROR();*/
