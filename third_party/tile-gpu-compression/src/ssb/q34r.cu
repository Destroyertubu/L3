...
where lo_custkey = c_custkey
  ...
  and (c_city = 'UNITED KI1' or c_city = 'UNITED KI5')  -- city IN {249, 244}
  and (s_city = 'UNITED KI1' or s_city = 'UNITED KI5')  -- city IN {249, 244}
  and d_yearmonth = 'Dec1997'  -- datekey IN [19971201, 19971231]
...
constexpr int UK1 = 249;  // UNITED KI1
constexpr int UK5 = 244;  // UNITED KI5
...
__global__ void probe(
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items)) {
      if (selection_flags[ITEM]) {
        // Aggregation by (year, c_city, s_city) - 6 years × 250 cities × 250 cities
        int hash = (s_city[ITEM] * NUM_CITIES * 6 + c_city[ITEM] * 6 + (year[ITEM] - 1992)) % (6 * NUM_CITIES * NUM_CITIES);
...
// Q34: s_city IN {249, 244}, return s_city
__global__
void build_hashtable_s(int *filter_col, int *dim_key, int num_tuples, int *hash_table, int num_slots) {
  ...
  if (offset < num_tuples) {
    int city = filter_col[offset];
    if (city == UK1 || city == UK5) {  // city IN {249, 244}
      int key = dim_key[offset];
      int hash = HASH(key, num_slots);
      int init = 0;

      int old = atomicCAS(&hash_table[hash << 1], init, key);
      hash_table[(hash << 1) + 1] = city;  // store city as value
    }
  }
}

// Q34: c_city IN {249, 244}, return c_city
__global__
...
// Q34: d_datekey IN [19971201, 19971231] (Dec1997)
__global__
void build_hashtable_d(int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots, int val_min) {
  ...
  if (offset < num_tuples) {
    ...
    // Filter on datekey for Dec1997
    if (key >= 19971201 && key <= 19971231) {
      int val = dim_val[offset];  // year
...
float runQuery(encoded_column lo_orderdate_val, encoded_column lo_orderdate_rl, encoded_column lo_custkey_val, encoded_column lo_custkey_rl,
  ...
  // Q34: filter on s_city IN {249, 244}
  build_hashtable_s<<<(s_len + 127)/128, 128>>>(s_city, s_suppkey, s_len, ht_s, s_len);

  // Q34: filter on c_city IN {249, 244}
  build_hashtable_c<<<(c_len + 127)/128, 128>>>(c_city, c_custkey, c_len, ht_c, c_len);

  // Q34: filter on d_datekey IN [19971201, 19971231] (Dec1997)
  int d_val_min = 19920101;
  build_hashtable_d<<<(d_len + 127)/128, 128>>>(d_datekey, d_year, d_len, ht_d, d_val_len, d_val_min);

  int *res;
  int res_size = 6 * NUM_CITIES * NUM_CITIES;  // 6 years × 250 cities × 250 cities
