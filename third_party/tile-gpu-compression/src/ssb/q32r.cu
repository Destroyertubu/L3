...
// Q32: s_nation = 24 (US), return s_city
__global__
void build_hashtable_s(int *filter_col, int *dim_key, int *dim_val, int num_tuples, int *hash_table, int num_slots) {
  ...
  if (offset < num_tuples) {
    if (filter_col[offset] == 24) {  // UNITED STATES = 24
...
// Q32: c_nation = 24 (US), return c_city
__global__
...
float runQuery(encoded_column lo_orderdate_val, encoded_column lo_orderdate_rl, encoded_column lo_custkey_val, encoded_column lo_custkey_rl,
  ...
  // Q32: filter on s_nation=24, return s_city
  build_hashtable_s<<<(s_len + 127)/128, 128>>>(s_nation, s_suppkey, s_city, s_len, ht_s, s_len);

  // Q32: filter on c_nation=24, return c_city
