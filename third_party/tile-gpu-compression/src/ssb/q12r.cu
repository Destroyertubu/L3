...
__global__ void QueryKernel(
  ...
  // Q1.2: d_yearmonthnum = 199401 (equivalent to orderdate between 19940101 and 19940131)
  #pragma unroll
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    ...
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items))
      selection_flags[ITEM] = (items[ITEM] >= 19940101 && items[ITEM] <= 19940131);
  }
  ...
  // Q1.2: lo_quantity between 26 and 35
  #pragma unroll
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items))
      selection_flags[ITEM] = selection_flags[ITEM] && (items[ITEM] >= 26 && items[ITEM] <= 35);
  }
  ...
  // Q1.2: lo_discount between 4 and 6
  #pragma unroll
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
  {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items))
      selection_flags[ITEM] = selection_flags[ITEM] && (items[ITEM] >= 4 && items[ITEM] <= 6);
  }
