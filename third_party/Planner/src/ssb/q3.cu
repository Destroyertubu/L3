...
DecompressedQ3Ptrs decompress_q3_cols(const Q3EncodedLO& cols,
...
void run_q31(const RunOptions& opt) {
    std::cout << "=== Q31 (Planner - Cascading Decompression) ===" << std::endl;
    ...
    // Warmup (decode + probe, not reported).
    {
        ...
        decomp_timer.start();
        auto ptrs = decompress_q3_cols(cols,
                                       d_lo_orderdate,
                                       d_lo_custkey,
                                       d_lo_suppkey,
                                       d_lo_revenue,
...
void run_q32(const RunOptions& opt) {
    std::cout << "=== Q32 (Planner - Cascading Decompression) ===" << std::endl;
    ...
    for (int run = 0; run < opt.runs; ++run) {
        ...
        timer.start();
        auto ptrs = decompress_q3_cols(cols, d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_revenue,
                                       ws_orderdate, ws_custkey, ws_suppkey, ws_revenue,
                                       ws_orderdate_rle, ws_custkey_rle, ws_suppkey_rle, ws_revenue_rle);

        probe_q32<BLOCK_THREADS, ITEMS_PER_THREAD>
            <<<static_cast<int>((LO_LEN + TILE_ITEMS - 1) / TILE_ITEMS), BLOCK_THREADS>>>(
                ptrs.orderdate, ptrs.custkey, ptrs.suppkey, ptrs.revenue, static_cast<int>(LO_LEN),
...
void run_q33(const RunOptions& opt) {
    std::cout << "=== Q33 (Planner - Cascading Decompression) ===" << std::endl;
...
void run_q34(const RunOptions& opt) {
    std::cout << "=== Q34 (Planner - Cascading Decompression) ===" << std::endl;
